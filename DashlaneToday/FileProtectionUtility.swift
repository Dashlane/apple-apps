...
public final class FileProtectionUtility: NSObject {
    ...
    override init() {
        ...
        NotificationCenter.default.addObserver(forName: UIApplication.protectedDataDidBecomeAvailableNotification, object: nil, queue: OperationQueue.main, using: { (notification) in
            print("data did become available")
            self.lockState.transition(toState: .unlockNotification)
        })
        NotificationCenter.default.addObserver(forName: UIApplication.protectedDataWillBecomeUnavailableNotification, object: nil, queue: OperationQueue.main, using: { (notification) in
