...
extension String {
  ...
  fileprivate var groupedBy3: String {
    assert(self.count == 6)
...
open class TodayViewController: UITableViewController, NCWidgetProviding {
  ...
  {
    let cell =
      tableView.dequeueReusableCell(withIdentifier: "TokenCell", for: indexPath) as! TokenCell
  ...
  override open func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
    ...
    let subtitle =
      context.isUniversalClipboardEnabled ? "TODAY_ON_YOUR_IPHONE_AND_MAC".localized : nil
  ...
  public func widgetActiveDisplayModeDidChange(
    _ activeDisplayMode: NCWidgetDisplayMode, withMaximumSize maxSize: CGSize
  ) {
    ...
    case .expanded:
      ...
      preferredContentSize = CGSize(
        width: maxSize.width, height: CGFloat(rows) * tableView.rowHeight)
    case .compact:
  ...
  private func startTimer() {
    ...
    timer = Timer.scheduledTimer(
      timeInterval: 1.0, target: self, selector: #selector(tic), userInfo: nil, repeats: true)
