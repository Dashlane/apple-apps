...
extension AppCoordinator {
  func createSessionFromUITestCommandIfNeeded() {
    Task {
      ...
      let listener = IPCMessageListener<SessionConfiguration>(
        urlToObserve: url,
  ...
  func login(using configuration: SessionConfiguration) async throws {
    ...
    let session = try appServices.sessionContainer.createSession(
      with: configuration, cryptoConfig: .masterPasswordBasedDefault)
    let services = try await withCheckedThrowingContinuation { continuation in
      sessionServicesSubscription = SessionServicesContainer.buildSessionServices(
        from: session,
