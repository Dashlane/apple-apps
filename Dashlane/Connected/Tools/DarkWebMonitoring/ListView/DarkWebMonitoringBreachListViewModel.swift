...
class DarkWebMonitoringBreachListViewModel: ObservableObject, SessionServicesInjecting {
    ...
    var shouldShowList: Bool {
        return registeredEmails.first(where: { $0.state == .active }) != nil
    }
    ...
    func reportPendingBreaches() {
        let pendingBreaches = breaches.filter { BreachListType.pending.storedBreachStatuses.contains($0.status) }

        pendingBreaches.forEach {
            activityReporter.report(UserEvent.ReceiveSecurityAlert(itemTypesAffected: [.securityBreach],
...
extension DarkWebMonitoringBreachListViewModel {
    static func mock(breaches: [DWMSimplifiedBreach]? = nil, isMonitoringAvailable: Bool = true) -> DarkWebMonitoringBreachListViewModel {
        let model = DarkWebMonitoringBreachListViewModel(darkWebMonitoringService: DarkWebMonitoringServiceMock(isDwmEnabled: isMonitoringAvailable),
