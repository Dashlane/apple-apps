...
struct DarkWebToolsFlow: View {
    ...
    var body: some View {
        StepBasedContentNavigationView(steps: $viewModel.steps) { step in
            switch step {
            case .root:
                DarkWebMonitoringView(model: viewModel.darkWebMonitoringViewModelFactory.make(actionPublisher: viewModel.actionPublisher))
                    .onAppear {
                        viewModel.appeared()
                    }
            case let .detail(breach):
                DarkWebMonitoringDetailsView(model: viewModel.makeDarkWebMonitoringDetailsViewModel(for: breach))
            case let .credentialDetails(model):
        ...
        .sheet(item: $viewModel.presentedSheet) { sheet in
            switch sheet {
            case .addEmail:
                DataLeakMonitoringAddEmailView(viewModel: viewModel.makeDataLeakMonitoringAddEmailViewModel())
            }
        }
    }
}
