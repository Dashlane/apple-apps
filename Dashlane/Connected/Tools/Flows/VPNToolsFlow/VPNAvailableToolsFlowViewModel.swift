...
private extension VPNAvailableToolsFlowViewModel {

    func handleAccountActivated() {
    ...
    func showStoreProduct() {
        ...
        if UIApplication.shared.canOpenURL(url) {
            ...
            if Device.isMac {
                ...
                guard let appURL = URL(string: appLink), UIApplication.shared.canOpenURL(appURL) else { return }
                UIApplication.shared.open(appURL, options: [:], completionHandler: nil)
            } else {
                guard !isDisplayingStoreProductViewController else { return }

                isDisplayingStoreProductViewController = true
                let parameters = [SKStoreProductParameterITunesItemIdentifier: "\(VPNService.vpnExternalAppId)"]
                skStoreProductViewController.loadProduct(withParameters: parameters) { [weak self] (loaded, _) -> Void in
                    guard let self = self, loaded else { return }
                    UIApplication.shared.keyUIWindow?.rootViewController?.present(self.skStoreProductViewController, animated: true, completion: nil)
                }
            }
        }
...
extension VPNAvailableToolsFlowViewModel {
    static var mock: VPNAvailableToolsFlowViewModel {
        VPNAvailableToolsFlowViewModel(session: .mock,
