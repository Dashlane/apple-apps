...
struct VPNAvailableToolsFlow: View {
    ...
    var body: some View {
        StepBasedContentNavigationView(steps: $viewModel.steps) { step in
            switch step {
            case .root:
                VPNMainView(model: viewModel.vpnMainViewModelFactory.make(mode: .activationNeeded, actionPublisher: viewModel.actionPublisher))
            case .accountActivation:
                VPNActivationView(model: viewModel.vpnActivationViewModelFactory.make(actionPublisher: viewModel.actionPublisher))
            }
        }
    }
}
