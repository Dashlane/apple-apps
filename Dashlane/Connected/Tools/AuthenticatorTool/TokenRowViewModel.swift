...
class TokenRowViewModel: ObservableObject {
    let token: OTPInfo
    let title: String
    let subtitle: String
    ...
    init(token: OTPInfo,
        ...
        self.title = token.configuration.issuerOrTitle.removingPercentEncoding ?? token.configuration.issuerOrTitle
        self.isDashlaneToken = token.isDashlaneOTP
        self.dashlaneTokenCaption = dashlaneTokenCaption
        if let parsed = domainParser.parse(urlString: isDashlaneToken ? "dashlane.com" : token.configuration.iconURL), parsed.publicSuffix != nil {
            domain = parsed
        } else {
            domain = nil
        }
