...
struct OTPTokenListView: View {
    ...
    init(viewModel: @autoclosure @escaping () -> OTPTokenListViewModel,
    ...
    var listView: some View {
        VStack(spacing: isEditing ? 8 : 0) {
            ExpandableForEach(viewModel.tokens,
                              id: \.id,
        ...
        }.background(RoundedRectangle(cornerRadius: 8).foregroundColor(.ds.container.agnostic.neutral.supershy))
    ...
    var body: some View {
        ...
        VStack(spacing: 24) {
            ...
            actionButtons.hidden(isListExpanded || isEditing)
        } .padding()
        }
    ...
    var actionButtons: some View {
        VStack {
            RoundedButton(L10n.Localizable.otpToolSetupCta, action: viewModel.startSetupOTPFlow)
                .roundedButtonLayout(.fill)
            Button.init(action: viewModel.startExplorer, title: L10n.Localizable.otpToolExploreAuthenticator)
    ...
    func deletionAlert(for item: OTPInfo) -> Alert {
        Alert(title: Text(L10n.Localizable.otptoolDeletionAlertTitle),
              message: Text(L10n.Localizable.otptollItemDeletionAlertTitle(item.configuration.issuerOrTitle)),
              primaryButton: .destructive(Text(L10n.Localizable.otptollItemDeletionAlertYes), action: { viewModel.delete(item: item) }),
