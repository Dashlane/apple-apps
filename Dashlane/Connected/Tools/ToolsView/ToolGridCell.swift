...
struct ToolsViewCellView_Previews: PreviewProvider {
    static var previews: some View {
        MultiContextPreview(dynamicTypePreview: true) {
            VStack(spacing: 10) {
                ToolGridCell(tool: ToolInfo(item: .secureWifi, state: .available(beta: false)))
                    .frame(minHeight: 112)
                ToolGridCell(tool: ToolInfo(item: .secureWifi, state: .needsUpgrade))
                    .frame(minHeight: 112)
                ToolGridCell(tool: ToolInfo(item: .secureWifi, state: .available(beta: true)))
                    .frame(minHeight: 112)
            }
