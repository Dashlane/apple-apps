...
struct SecurityBreachAlert {
  ...
  static func alert(
    for popupAlert: PopupAlertProtocol,
    withCompletion completion: @escaping SecurityBreachAlertHandler
  ) -> (controller: UIAlertController, popupAlert: PopupAlertProtocol) {
    ...
    buttons.forEach { button in
      ...
      alert.addAction(
        UIAlertAction(
          title: button.localized, style: style,
          handler: { _ in
            completion(button)
          }))
    }
