...
extension UIKeyCommand {
  ...
  fileprivate static func makeSyncInfoItem(
    isSyncing: Bool, lastSync: Date, formatter: DateFormatter
  ) -> UICommand {
    let title: String
    if isSyncing {
  ...
  @objc fileprivate func emptyFunction() {}
  ...
  fileprivate static func makeSyncItem(enable: Bool) -> UIKeyCommand {
    ...
    return UIKeyCommand(
      title: L10n.Localizable.keyboardShortcutSyncNow,
      image: nil,
      action: selector,
      input: "R",
