...
class SessionMainMenuHandler: MainMenuHandler {
  ...
  init(
    applicationHandler: ApplicationMainMenuHandler, syncService: SyncService,
    bridge: MainMenuBarBridge, logger: Logger
  ) {
  ...
  func handle(_ command: UICommand) -> Bool {
    if let authenticatedCommand = SessionDefaultShortcutCommand(
      fromPropertyList: command.propertyList)
    {
