...
struct MatchingCredentialsListView: View {
    ...
    private var explanations: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                Text(L10n.Localizable.pwmMatchingCredentialsListMultipleLoginsAvailable(viewModel.matchingCredentials.count, viewModel.issuer))
                    .font(.title.bold())
    ...
    private func row(for credential: Credential) -> some View {
        HStack(spacing: 16) {
            VaultItemRow(model: viewModel.vaultItemRowModelFactory.make(
