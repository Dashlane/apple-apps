...
extension VaultFlowViewModel {
    ...
    func canHandle(deepLink: VaultDeeplink) -> Bool {
        ...
        switch deepLink {
        case let .list(category):
            if Device.isIpadOrMac, let category = category {
                showCategoryDetail(category)
            } else {
                showList(for: category)
            }
        case let .fetchAndShow(identifier, useEditMode):
                        guard let item = sessionServices.vaultItemsService.item(for: identifier) else { return }
    ...
    private func editMode(item: VaultItem, editMode: Bool, origin: ItemDetailOrigin) -> ItemDetailViewType {
        return editMode ? .editing(item) : .viewing(item, actionPublisher: actionPublisher, origin: origin)
    }
}

private extension VaultItemsService {

                    func item(for identifier: VaultDeepLinkIdentifier) -> VaultItem? {
