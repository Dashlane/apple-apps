...
enum ItemCategory {
  ...
  func count(personalDataService: PersonalDataService) -> Int {
    ...
    case .personalInfo:
      ...
      return [
        identitiesCount, emailsCount, phonesCount, addressesCount, companiesCount,
        personalWebsitesCount,
      ].reduce(0, +)
    case .ids:
      ...
      let socialSecuritiesCount =
        (try? personalDataService.count(for: SocialSecurityInformation.self)) ?? 0
      ...
      return [
        passportsCount, drivingLicensesCount, socialSecuritiesCount, idCardsCount, fiscalInfoCount,
      ].reduce(0, +)
    }
  }

  func makeViewModel(
    from sessionServices: SessionServicesContainer,
  ...
  var itemTypes: [PersonalDataItem.Type] {
    ...
    case .personalInfo:
      return [
        Identity.self, Email.self, Phone.self, Address.self, Company.self, PersonalWebsite.self,
      ]
    case .ids:
      return [
        Passport.self, DrivingLicence.self, SocialSecurityInformation.self, IDCard.self,
