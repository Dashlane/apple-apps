...
struct CredentialDomainsView: View {
    ...
    init(model: CredentialDomainsViewModel, addedDomains: [LinkedServices.AssociatedDomain]) {
        self.model = model
        self._addedDomains = State(initialValue: addedDomains.map {
    ...
    var body: some View {
        ...
        .onChange(of: domainIdToEdit, perform: { domainIdToCheck in
            ...
            if model.isAdditionMode {
                ...
                NavigationBarButton(L10n.Localizable.kwSave) {
                    checkDuplicate(of: lastIdDuplicateChecked) { domains in
                        ...
                        model.save(addedDomains: LinkedServices(associatedDomains: domainsToSave.map { $0.content }))
                    }
                }
                .hidden(model.initialMode.isEditing)
            }
    ...
    var servicesUpdating: some View {
        Section(header: Text(L10n.Localizable.KWAuthentifiantIOS.Domains.addedByYou.uppercased())) {
            ...
            Button(action: {
                ...
                let domainToAppend = EditableDomain(content: LinkedServices.AssociatedDomain(domain: "", source: .manual))
    ...
    private func commit(domains: [EditableDomain]) {
        ...
        model.commit(addedDomains: LinkedServices(associatedDomains: domainsToCommit.map { $0.content }))
    }
    ...
    private func checkDuplicate(of uuid: UUID?, completion: @escaping ([EditableDomain]) -> Void = {_ in }) {
        guard let addedDomain = addedDomains.first(where: { $0.id == uuid }), duplicatedCredential == nil else {
            completion(self.addedDomains)
            return
        }

                if let duplicate = model.hasDuplicate(for: addedDomain.content.domain) {
            duplicatedCredential = DuplicatePrompt(domain: addedDomain, title: duplicate.displayTitle, completion: completion)
            return
        }

                if addedDomains.filter({ $0.content.domain == addedDomain.content.domain }).count > 1 {
            duplicatedCredential = DuplicatePrompt(domain: addedDomain, title: model.item.displayTitle, completion: completion)
            return
        }
...
struct CredentialDomainsView_Previews: PreviewProvider {
    ...
    static let linkedServices: [LinkedServices.AssociatedDomain] = [LinkedServices.AssociatedDomain(domain: "live.com", source: DomainSource.remember), LinkedServices.AssociatedDomain(domain: "outlook.com", source: DomainSource.manual)]

    func model(mode: DetailMode) -> CredentialDomainsViewModel {
        return CredentialDomainsViewModel(item: CredentialDomainsView_Previews.credential,
