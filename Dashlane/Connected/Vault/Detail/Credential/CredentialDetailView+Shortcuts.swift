...
extension View {
    ...
    func makeShortcuts(model: CredentialDetailViewModel) -> some View {
        self
            .mainMenuShortcut(.copyPrimary(title: L10n.Localizable.copyPassword),
                              enabled: !model.mode.isEditing && !model.item.password.isEmpty && model.item.metadata.sharingPermission != .limited,
                              action: { model.copy(model.item.password, fieldType: .password) })
            .mainMenuShortcut(.copySecondary(title: L10n.Localizable.copyEmail),
                              enabled: !model.mode.isEditing && model.item.shouldEnableEmailShortcut,
                              action: { model.copy(model.item.email, fieldType: .email) })
            .mainMenuShortcut(.copySecondary(title: L10n.Localizable.copyLogin),
...
private extension Credential {
    var shouldEnableEmailShortcut: Bool {
        return !email.isEmpty
    }

    var shouldEnableLoginShortcut: Bool {
        return !shouldEnableEmailShortcut && !login.isEmpty
    }

    var shouldEnableSecondaryLoginShortcut: Bool {
