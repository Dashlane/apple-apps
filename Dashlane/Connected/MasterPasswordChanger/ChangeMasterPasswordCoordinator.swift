...
class ChangeMasterPasswordCoordinator: Coordinator {
  ...
  func start() {
    ...
    self.navigationController = self.navigator.presentAsModal(
      warningController, style: .fullScreen, barStyle: .default, animated: true)
  }
  ...
  private func makeWarningViewController() -> ChangeMasterPasswordWarningViewController {
    ...
    guard
      let viewController = storyboard.instantiateViewController(withIdentifier: identifier)
        as? ChangeMasterPasswordWarningViewController
    else {
  ...
  private func pushMasterPasswordInputViewController() {
    let model = NewMasterPasswordViewModel(
      mode: .masterPasswordChange, evaluator: sessionServices.appServices.passwordEvaluator,
      logger: nil
    ) { [weak self] result in
      switch result {
      case .back:
  ...
  func startChangingMasterPassword(with masterPassword: String) throws {
    changeMasterPasswordService = try createMasterPasswordChangerService(
      sessionServices: sessionServices, withNewMasterPassword: masterPassword)
    let changingMasterPasswordViewController = ChangingMasterPasswordViewController.instantiate(
...
extension ChangeMasterPasswordCoordinator {
  ...
  func createMasterPasswordChangerService(
  ...
  ) throws -> RemoteCryptoChangerService {

    let session = sessionServices.session

    let currentCryptoKey = session.configuration.cryptoKey

    let newCryptoKey = CryptoKey(
      masterKey: .masterPassword(newMasterPassword), serverKey: currentCryptoKey.serverKey)

    let newSession = try sessionServices.appServices.sessionContainer.createSession(
      for: newCryptoKey,
      from: session,
      isSSO: false,
      isSSOToMPMigration: false)
    let masterPasswordChangerCryptoEngine = CryptoChangerEngine(
      current: session.masterKeyCryptoEngine, new: newSession.masterKeyCryptoEngine)
    self.newSession = newSession

    let postMPChangeUpdaterService = PostMPChangeUpdaterService(
    ...
    return RemoteCryptoChangerService(
...
extension PremiumService {
  fileprivate func updatePremiumStatus(
    timeoutForRequest: TimeInterval, failure: @escaping () -> Void,
    success: @escaping (PremiumStatus) -> Void
  ) {
