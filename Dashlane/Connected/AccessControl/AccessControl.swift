...
extension AccessControlReason {
  var logReason: Definition.Reason {
    ...
    case .unlockItem:
      return .unlockItem
    case .lockOnExit, .changeContactEmail:
      return .editSettings
    }
  }
...
class AccessControl: AccessControlViewModelProtocol, AccessControlProtocol {
  ...
  init(
    session: Session, secureLockProvider: SecureLockProvider,
  ...
  private func makePendingAccess(reason: AccessControlReason) -> PendingAccess {
    ...
    case .biometry:
      accessMode = makePendingAccessForBiometry(
        for: reason, fallback: makePendingAccessForMasterPassword(reason: reason))
    case .pincode(let lock):
      accessMode = makePendingAccessForPin(code: lock.code, reason: reason)
    case .biometryAndPincode(_, let lock):
      accessMode = makePendingAccessForBiometry(
        for: reason, fallback: makePendingAccessForPin(code: lock.code, reason: reason))
  ...
  private func makePendingAccessForBiometry(
    for reason: AccessControlReason,
    fallback: Mode
  ) -> Mode {
    ...
    return .biometry { [weak self] in
      DispatchQueue.main.async {
        self?.validateBiometry(
          forReason: reason,
          fallbackTitle: fallback.isPin ? CoreLocalization.L10n.Core.enterPasscode : nil
        ) {
  ...
  private func makePendingAccessForPin(code: String, reason: AccessControlReason) -> Mode {
    ...
    return .pin(code.count) { [weak self] enteredPin in
  ...
  func requestAccess(forReason reason: AccessControlReason) -> AccessControlPublisher {
    ...
    return pendingAccess.publisher.receive(on: DispatchQueue.main).eraseToAnyPublisher()
  }
  ...
  private func validateBiometry(
    forReason reason: AccessControlReason, fallbackTitle: String? = nil,
  ...
  private func validateMasterPassword(_ password: String) {
    guard let masterPassword = self.session.authenticationMethod.userMasterPassword,
  ...
  private func logAskAuthentication(
    for mode: Definition.Mode,
