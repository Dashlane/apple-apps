...
struct IdentityBreachAlertFactory {
  ...
  private func singleBreachAlert(viewModel: IdentityBreachAlertViewModel, popup: PopupAlertProtocol)
    -> UIAlertController
  {
  ...
  private func groupedNotificationsAlert(viewModel: IdentityBreachAlertViewModel)
    -> UIAlertController
  {
    ...
    let message = NSMutableAttributedString(
      string: L10n.Localizable.securityBreachMultipleAlertDescription(numberOfBreaches))

    if let range = message.string.range(of: "\(numberOfBreaches)") {
      message.addAttributes(
        [NSAttributedString.Key.foregroundColor: UIColor.ds.text.brand.standard],
        range: NSRange(range, in: message.string))
    }

    let alert = UIAlertController(
      title: NSLocalizedString("SECURITY_BREACH_MULTIPLE_ALERT_TITLE", comment: ""),
      message: "",
    ...
    let laterAction = UIAlertAction(
      title: NSLocalizedString("SECURITY_BREACH_MULTIPLE_ALERT_CLOSE_CTA", comment: ""),
      style: .cancel
    ) { [viewModel] _ in
      viewModel.updateBreachesStatus(for: breachesId, to: .viewed)
    }
    let viewAction = UIAlertAction(
      title: NSLocalizedString("SECURITY_BREACH_MULTIPLE_ALERT_VIEW_CTA", comment: ""),
      style: .default
    ) { [viewModel] _ in
