...
class BreachesStore: SecurityDashboard.BreachesStore {
    ...
    var lastUpdateDateForDataLeakBreaches: TimeInterval? {
        get {
            let value: TimeInterval? = DispatchQueue.main.sync { self.settings[.lastUpdateDataForDataLeaks] }
            log.debug("lastRevisionForPublicBreaches used is \(value ?? -1)")
            return value
        }
        set {
    ...
    func breachesPublisher() -> AnyPublisher<Set<StoredBreach>, Never> {
        database.itemsPublisher(for: SecurityBreach.self)
            ...
            .map { breaches in
                let storedBreaches = breaches.compactMap { $0.storedBreach(using: self.breachContentDecoder) }
    ...
    func create(_ breaches: Set<StoredBreach>) {
        ...
        let securityBreaches = breaches.compactMap { SecurityBreach(storedBreach: $0, encoder: breachContentEncoder) }
    ...
    func update(_ breaches: Set<StoredBreach>) {
        ...
        let securityBreaches: [SecurityBreach] = breaches.compactMap {
            ...
            guard var securityBreach = try? database.fetch(with: identifier.id, type: SecurityBreach.self) else {
                return nil
            }
...
private extension Array where Element == StoredBreach {

            func merged() -> Set<StoredBreach> {
        ...
        return StoredBreach(objectID: objectID,
                            breach: base.breach,
...
private extension StoredBreach {
    var lastModificationRevision: Int {
