...
class IdentityDashboardWidgetService {
  func refresh(withReport report: SecurityDashboard.PasswordHealthReport) {
    ...
    ApplicationGroup.userDefaults.set(
      report.allCredentialsReport.totalCount, forKey: DashlaneWidgetConstant.credentialCount)
    ApplicationGroup.userDefaults.set(
      compromisedItems, forKey: DashlaneWidgetConstant.compromisedCount)
