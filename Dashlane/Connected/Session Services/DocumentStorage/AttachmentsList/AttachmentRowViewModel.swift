...
class AttachmentRowViewModel: ObservableObject, SessionServicesInjecting {
    ...
    let logger: AttachmentsListUsageLogger
    ...
    init(attachment: Attachment,
        ...
        self.logger = .init(anonId: editingItem.anonId, usageLogService: usageLogService)
    ...
    func getDownloadedFileURL() async throws -> URL {
        ...
        return try self.documentStorageService.documentCache.move(url,
    ...
    func renameAttachment() {
        ...
        do {
            ...
            self.attachment = temp
                        ...
                        if var secureFileInfo = try database.fetch(with: .init(temp.id), type: SecureFileInformation.self) {
...
private extension Sequence where Element == DocumentUpload {
    func progress(for attachment: Attachment) -> Progress? {
        first { $0.secureFileInfo.id.rawValue == attachment.id }?.progress
    }
}

private extension Sequence where Element == DocumentDownload {
    func progress(for attachment: Attachment) -> Progress? {
...
extension AttachmentRowViewModel {
    ...
    static var mock: AttachmentRowViewModel {
        .init(attachment: mockAttachment,
