...
class SpiegelStoreMigrater {
    ...
    var isRestoringMode: Bool {
        ...
        guard isRestoringMode || !fiberStoreExists else {
            logger.info("Nothing to do, store already exists and we are not in the forced restoring mode")
            return
        }
        ...
        guard let entitiesHistory = entitiesByName[HistoryEntity.dataChangeHistory.entityName],
            let entitiesChangeSet = entitiesByName[HistoryEntity.changeSet.entityName] else {
        ...
        do {
            ...
            let syncStatus = try spiegelDataBase
                .loadTransactionActions()
                .mapValues { $0.linkedSyncStatus }
            logger.info("pendingSync: \(syncStatus)")

            let regularNodes: [Node] = try spiegelDataBase
                .loadDataRows(for: model.entities.filter { $0 != entitiesHistory && $0 != entitiesChangeSet })
                .map { row in
                    Node(id: row.id,
    ...
    private func loadDatabase(locatedAt location: URL) throws -> SpiegelSQLCipheredDatabase {
        do {
            ...
            return try SpiegelSQLCipheredDatabase(filePath: location.absoluteString, storeEncryption: .key(localKey))
        } catch {
                        guard let password = self.session.configuration.masterKey.masterPassword else { throw error }
            return try SpiegelSQLCipheredDatabase(filePath: location.absoluteString, storeEncryption: .password(password))
        }
    }
        ...
        if let historyContent = content[HistoryEntity.dataChangeHistory.contentKey] as? [String: Any] {
            content[HistoryEntity.dataChangeHistory.contentKey] =
                Node(entityName: HistoryEntity.dataChangeHistory.entityName,
        ...
        private func transformHistoryContent(_ content: [String: Any], changeSetNodes: [Node]) -> [String: Any] {
        var changeSetsById = [String: [String: Any]]()
        for set in changeSetNodes {
            changeSetsById[set.id] = set.propertyCache.filter { $0.key != HistoryEntity.dataChangeHistory.contentKey }
        }

        var content = content
        if let changesSets = content[HistoryEntity.changeSet.contentKey] as? [String: Any] {
            content[HistoryEntity.changeSet.contentKey] = changesSets.compactMap { id, _ -> [AnyHashable: Any]? in
                guard let content = changeSetsById["{" + id + "}"] else {
                    return nil
                }
                return Node(entityName: HistoryEntity.changeSet.entityName,
                            propertyCache: content).jsonDictionary
            }
        }
...
extension SpiegelStoreMigrater {
    ...
    static func migratePublisher(session: Session,
                                 loadingContext: SessionLoadingContext,
                                 appSettings: AppSettings,
                                 storeURL: URL,
                                 logger: Logger,
                                 forceRestoringMode: Bool = false) -> Future<Void, Error> {
        ...
        return Future<Void, Error> { completion in
            ...
            DispatchQueue.global(qos: .userInitiated).async {
                let migrater = SpiegelStoreMigrater(session: session,
                                                    appSettings: appSettings,
...
private extension SyncTransactionActionType {
    var linkedSyncStatus: SyncStatus? {
        switch self {
            case .edit:
    ...
    func lastBackupDate() -> Timestamp {
        return reduce(Timestamp.distantPast) { result, node in
            guard let rawDateString = node.propertyCache["backupDate"] as? String,
                let rawDate = UInt64(rawDateString) else {
                    return result
            }
