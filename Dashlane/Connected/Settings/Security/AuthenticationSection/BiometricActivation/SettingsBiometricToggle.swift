...
struct SettingsBiometricToggle: View {
  ...
  private func makeMasterPasswordResetDeactivationWarningAlert(
    completion: @escaping (Confirmed) -> Void
  ) -> Alert {
    let title = L10n.Localizable.resetMasterPasswordBiometricsDeactivationDialogTitle(
      Device.currentBiometryDisplayableName)
    return Alert(
      title: Text(title),
      message: nil,
      primaryButton: .destructive(
        Text(L10n.Localizable.resetMasterPasswordBiometricsDeactivationDialogDisable),
        action: { completion(true) }),
      secondaryButton: .cancel(
        Text(L10n.Localizable.resetMasterPasswordBiometricsDeactivationDialogCancel),
        action: { completion(false) }))
  }

  private func makeMasterPasswordResetActivationSuggestionAlert(
    completion: @escaping (Confirmed) -> Void
  ) -> Alert {
