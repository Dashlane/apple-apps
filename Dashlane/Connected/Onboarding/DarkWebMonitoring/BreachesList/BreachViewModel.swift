...
class BreachViewModel: ObservableObject, SessionServicesInjecting {
    ...
    var displayDate: String {
        guard let date = leakDate else { return L10n.Localizable.darkWebMonitoringBreachViewDateUnknown }
    ...
    init(hasBeenAddressed: Bool,
        ...
        convenience init(breach: DWMSimplifiedBreach, iconViewModelProvider: @escaping (PersonalDataURL) -> DWMItemIconViewModel) {
        self.init(hasBeenAddressed: false, url: breach.url, leakedPassword: breach.leakedPassword, leakDate: breach.date, email: breach.email, otherLeakedData: breach.otherLeakedData, simplifiedBreach: breach, iconViewModelProvider: iconViewModelProvider)
    }

        convenience init(credential: Credential, iconViewModelProvider: @escaping (PersonalDataURL) -> DWMItemIconViewModel) {
        assert(credential.url != nil)
        self.init(hasBeenAddressed: true, url: credential.url ?? PersonalDataURL(rawValue: ""), leakedPassword: nil, leakDate: nil, iconViewModelProvider: iconViewModelProvider)
    }
        ...
        self.leakDate = nil
        self.iconViewModel = FakeDWMItemIconViewModel(url: credential.url ?? PersonalDataURL(rawValue: ""))
        self.email = nil
...
extension BreachViewModel {
    static func mock(for breach: DWMSimplifiedBreach,
                     hasBeenAddressed: Bool = false) -> BreachViewModel {
