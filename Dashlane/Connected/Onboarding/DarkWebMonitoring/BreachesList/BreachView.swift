...
struct BreachView<Model: BreachViewModel>: View {
  ...
  private var info: some View {
    Text(model.label)
      .font(.footnote)
      .foregroundColor(
        model.hasBeenAddressed ? .ds.text.positive.standard : .ds.text.warning.standard)
  }
}
...
struct BreachView_Previews: PreviewProvider {
  ...
  static var breachWithPassword: DWMSimplifiedBreach {
    return DWMSimplifiedBreach(
      breachId: "test1", url: PersonalDataURL(rawValue: "test1.com"), leakedPassword: "test",
      date: nil)
  }

  static var breachWithoutPassword: DWMSimplifiedBreach {
    return DWMSimplifiedBreach(
      breachId: "test2", url: PersonalDataURL(rawValue: "test2.com"), leakedPassword: nil, date: nil
    )
  }
