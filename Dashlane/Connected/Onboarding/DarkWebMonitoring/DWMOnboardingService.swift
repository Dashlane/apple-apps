...
class DWMOnboardingService {
  ...
  var shouldShowBreachesNotFoundInImportMethodsView: Bool {
    guard settings[.hasConfirmedEmailFromOnboardingChecklist] == true,
      settings[.progress] == DWMOnboardingProgress.breachesNotFound,
      settings[.hasSeenUnexpectedError] != true
    else { return false }
    return true
  }

  var hasGoneStraightToDWMOnboarding: Bool {
    settings[.hasGoneStraightToDWMOnboarding] == true
  }
    ...
    return
      ...
      .mapError { [weak self] error -> DWMEmailRegistrationError in
        if case let .unexpectedError(error) = error {
          self?.logger.error("Email registration error", error: error)
        }
        return error
      }
  ...
  func getState(_ notification: Notification? = nil, email: String) -> AnyPublisher<
    DataLeakEmail.State, EmailStateCheckError
  > {
  ...
  func state(
    forEmail: String,
    completion: @escaping (Result<DataLeakEmail.State, EmailStateCheckError>) -> Void
  ) {
  ...
  func handleResult(_ result: Result<DataLeakStatusResponse, Error>) -> Result<
    DataLeakEmail.State, EmailStateCheckError
  > {
    switch result {
    case .success(let response):
      if let email = response.emails.first,
        let state = DataLeakEmail.State(rawValue: email.state)
      {
        if state == .disabled {
          self.logger.error("Email for registration is unexpectedly disabled.")
        }
        return .success(state)
      } else {
        self.logger.error("Email registration status check: empty response, one email is expected.")
        return .failure(
          .unexpectedError(
            details: "Email registration status check: empty response, one email is expected."))
      }
    case .failure(let error):
      if error.isConnectionError {
        return .failure(.connectionError)
      } else {
        self.logger.error("Email status check error", error: error)
  ...
  private func breachesUpdatePublisher() -> AnyPublisher<
    (IdentityDashboardService.DataLeaksUpdate, [DWMSimplifiedBreach]), Never
  > {
    let registeredEmailPublisher: AnyPublisher<String, Never> = settings.publisher(
      for: .registeredEmail
    )
    .compactMap { $0 }
    .eraseToAnyPublisher()

    let dataLeakUpdateForRegisteredEmailPublisher = identityDashboardService
      .dataLeaksLastUpdatePublisher.combineLatest(registeredEmailPublisher)
    ...
    let debouncedBreachesPublisher = $pendingBreaches.debounce(
      for: 2, scheduler: DispatchQueue.main)

    return
      dataLeakUpdateForRegisteredEmailPublisher
  ...
  private func setupSecuredItemsIdsSubscription() {
    let securedItemsIdsPublisher: AnyPublisher<[String]?, Never> =
      settings.changeMonitoringPublisher(key: .securedItemsIds)
  ...
  func progressPublisher() -> AnyPublisher<DWMOnboardingProgress?, Never> {
    return settings.changeMonitoringPublisher(key: .progress).prepend(settings[.progress])
      .eraseToAnyPublisher()
  }
}

extension IdentityDashboardServiceProtocol {
  fileprivate var hasLastDataLeaksUpdateFinishedWithError: Bool {
    return dataLeaksLastUpdate.error != nil
  }

  fileprivate func refreshDataLeaks() {
