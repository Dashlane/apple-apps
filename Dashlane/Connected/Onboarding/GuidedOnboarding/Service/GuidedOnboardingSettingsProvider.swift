...
class GuidedOnboardingSettingsProvider: GuidedOnboardingDataProvider {
    ...
    var storedAnswers: [GuidedOnboardingQuestion: GuidedOnboardingAnswer] {
        guard let savedAnswers: [GuidedOnboardingSettingsData] = userSettings[.guidedOnboardingData] else {
            return [:]
        }
        ...
        return savedAnswers
            .map { data -> (GuidedOnboardingQuestion, GuidedOnboardingAnswer)? in
                guard let question = GuidedOnboardingQuestion(rawValue: data.questionId),
                    let answerId = data.answerId, let answer = GuidedOnboardingAnswer(rawValue: answerId) else {
                        return nil
                }
    ...
    init(userSettings: UserSettings) {
        ...
        NSKeyedUnarchiver.setClass(GuidedOnboardingSettingsData.self, forClassName: "Dashlane.GuidedOnboardingSettingsData")
        NSKeyedUnarchiver.setClass(GuidedOnboardingSettingsData.self, forClassName: "DashlaneAppKit.GuidedOnboardingSettingsData")
    }

    func storeAnswers(answers: [GuidedOnboardingQuestion: GuidedOnboardingAnswer]) {
        let updatedSelectedAnswers = answers
        let convertibleAnswers = updatedSelectedAnswers.map { (keyValue) -> GuidedOnboardingSettingsData in
