...
class GuidedOnboardingTransitionHandler {
  ...
  func update(state: TransitionState) {
    ...
    case .changed(let value):
      ...
      interactionController?.update(
        percentCompleted(fromValue: value, relativeToView: navigationController.view))
    case .end:
  ...
  private func percentCompleted(fromValue value: DragGesture.Value, relativeToView view: UIView)
    -> CGFloat
  {
