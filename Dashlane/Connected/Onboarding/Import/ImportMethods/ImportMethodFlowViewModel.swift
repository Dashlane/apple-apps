...
final class ImportMethodFlowViewModel: ObservableObject, SessionServicesInjecting {
    ...
    private func start() {
        steps = [
            .importView(importMethodViewModelFactory.make(
    ...
    private func showAddPassword() {
        let viewModel = addItemFlowViewModelFactory.make(displayMode: .categoryDetail(.credentials)) { [weak self] completion in
            switch completion {
            case .dismiss:
