...
public struct SecureLockConfigurator {
  ...
  init(
    session: Session, keychainService: AuthenticationKeychainServiceProtocol,
    settings: UserLockSettings
  ) {
  ...
  private func saveMasterKeyForBiometry() throws {
    try keychainService.saveMasterKey(
...
extension SecureLockConfigurator {
  ...
  func disableBiometry() throws {
    if settings[.pinCode] == true, let pincode = try? keychainService.pincode(for: session.login) {
      ...
      try setConvenientAuthentication(
        isRememberMasterPasswordActivated ? .rememberMasterPassword : .none)
    }
  }
}
