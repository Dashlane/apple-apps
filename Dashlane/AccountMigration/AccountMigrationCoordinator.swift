...
class AccountMigrationCoordinator: NSObject, Coordinator, SubcoordinatorOwner {
  ...
  init(
    ...
    loginKitServices = sessionServices.appServices.makeLoginKitServicesContainer()
  }
  ...
  private func startMasterPasswordToMasterPassword() {
    let isSyncEnabled = sessionServices.capabilityService.status(of: .sync).isAvailable
    self.mpNavigator.setRootNavigation(
      self.makeMPToMPMigrationView(
        navigator: self.mpNavigator,
        title: isSyncEnabled
    ...
    self.navigator.present(
      self.mpNavigator, animated: true,
      completion: {

      })
  }

  private func makeMPToSSOMigrationView(validator: SSOAuthenticationInfo) -> SSOMigrationView {
  ...
  private func makeSSOToMPMigrationView(validator: SSOAuthenticationInfo)
    -> MasterPasswordMigrationView
  {
    let view = MasterPasswordMigrationView(
      ...
      cancelButtonTitle: CoreLocalization.L10n.Core.kwLogOut
    ) { result in
      switch result {
      case .cancel:
        self.navigator.dismiss(animated: true)
      ...
      case .migrate:
        if let authTicket = self.authTicket {
          self.showMasterPasswordCreationView(
            navigator: self.navigator, authTicket: authTicket, type: .ssoToMasterPassword)
        } else {
  ...
  private func makeMPToMPMigrationView(navigator: Navigator, title: String, subtitle: String)
    -> MasterPasswordMigrationView
  {
    let view = MasterPasswordMigrationView(
      title: title,
      ...
      switch result {
      case .cancel:
        self.sessionServices.activityReporter.report(
          UserEvent.ChangeMasterPassword(flowStep: .cancel))
        self.navigator.dismiss(animated: true)
        self.completion(.success(.cancel))
      case .migrate:
        self.sessionServices.activityReporter.report(
          UserEvent.ChangeMasterPassword(flowStep: .start))
        self.showMasterPasswordCreationView(
          navigator: navigator, authTicket: nil, type: .masterPasswordToMasterPassword)
      }
    }
    return view
  }

  private func showMasterPasswordCreationView(
    navigator: Navigator, authTicket: String?, type: AccountMigrationType
  ) {
    let model = NewMasterPasswordViewModel(
      ...
      case let .next(masterPassword: masterPassword):
        Task {
          await self?.startChangingMasterPassword(
            with: masterPassword,
  ...
  private func startChangingMasterPassword(
    ...
    if let authTicket = authTicket, type == .ssoToMasterPassword {
      newAuthTicket = AuthTicket(token: authTicket, verification: Verification(type: .emailToken))
    }
    do {
      self.accountCryptoChangerService = try await createMasterPasswordChangerService(
        withNewMasterPassword: masterPassword,
        sessionServices: sessionServices,
        newVerification: type == .ssoToMasterPassword ? Verification(type: .emailToken) : nil,
  ...
  func startChangeMasterKey(withAuthTicket authTicket: String, serviceProviderKey: String) {
    do {
      ...
      accountCryptoChangerService = try self.createMasterPasswordChangerService(
        withServiceProviderKey: serviceProviderKey,
