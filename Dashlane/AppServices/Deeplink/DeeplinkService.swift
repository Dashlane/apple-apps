...
class DeepLinkingService: DeepLinkingServiceProtocol {
  ...
  func handle(_ userActivity: NSUserActivity) {
    ...
    else if userActivity.isSearchContinuation,
      let query = userActivity.userInfo?[CSSearchQueryString] as? String
    {
      ...
      guard let identifier = userActivity.userInfo?[CSSearchableItemActivityIdentifier] as? String,
        let deepLink = DeepLink(userActivityDeepLink: identifier)
      else {
        return
      }
      handleLink(deepLink)
...
extension DeepLinkingService {
  private class FakeDeepLinkingService: DeepLinkingServiceProtocol, VaultKit
      .DeepLinkingServiceProtocol
  {
