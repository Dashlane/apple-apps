            ...
            switch pathComponents.count {
                ...
                case 2:
                    if let action = ItemActionDeepLinkComponent(rawValue: pathComponents[1]) {
                        ...
                        self = .vault(.fetchAndShow(.init(rawIdentifier: pathComponents[1], component: vaultItemComponent), useEditMode: false))
                    }
                case 3:
                    guard let action =  ItemActionDeepLinkComponent(rawValue: pathComponents[2]), action == .edit  else {
                        return nil
                    }

                    self = .vault(.fetchAndShow(.init(rawIdentifier: pathComponents[1], component: vaultItemComponent), useEditMode: true))
                default:
                    return nil
            }
        } else if let settings = SettingsDeepLinkComponent(pathComponents: pathComponents, queryParameters: queryParameters) {
            self = .settings(settings)
        } else if let userNotConnected = UserNotConnectedDeepLink(pathComponents: pathComponents, queryParameters: queryParameters) {
            self = .userNotConnected(userNotConnected)
        } else if let importMethod = ImportMethodDeeplink(pathComponents: pathComponents, queryParameters: queryParameters) {
