...
extension DeepLinkingService {
  ...
  func configureDeeplinksFromNotifications() {
    configurePremium()
    configureTokenAndDevice()
    configureSharing()
    configureLocalNotification()
    configureBraze()
    configureDarkWebMonitoring()
  }

  private func configurePremium() {
    notificationService.userNotificationPublisher(for: .code(.trialUser)).onlyResponse().sink {
      [weak self] (response, completion) in
    ...
    notificationService.userNotificationPublisher(for: .code(.renewal)).onlyResponse().sink {
      [weak self] (response, completion) in
      if let deepLink = response.notification.deepLink {
        self?.handleLink(deepLink)
      }
  ...
  private func configureTokenAndDevice() {
    notificationService.userNotificationPublisher(for: .code(.token)).sink { [weak self] event in
      ...
      case let .willPresent(notification, completionHandler):
        completionHandler(notification.securityToken != nil ? [.list, .banner] : [])
    ...
    notificationService.userNotificationPublisher(for: .code(.general)).onlyResponse().sink {
      [weak self] (response, completion) in
      let postNotificationValue = response.notification[
        infoKey: NotificationInfoKey.postNotificationValue, type: String.self]
  ...
  private func configureSharing() {
    notificationService.userNotificationPublisher(for: .code(.sharingEventItemGroup)).sink {
      [weak self] event in
      switch event {
      case let .readDelivered(_, completionHandler):
        completionHandler(.delete)
      ...
      case let .willPresent(_, completionHandler):
        completionHandler(
          self?.sessionLifeCycleHandler?.sessionState.isContentVisible == true
            ? [] : [.list, .banner])
  ...
  private func configureLocalNotification() {
    notificationService.userNotificationPublisher(for: .local(.tachyonOTPDisplay)).onlyResponse()
    ...
    notificationService.userNotificationPublisher(for: .local(.autofillReminder)).onlyResponse()
      .sink { [weak self] (response, completion) in
        if let deepLink = response.notification.deepLink {
          self?.handleLink(deepLink)
        }
  ...
  private func configureBraze() {
    notificationService.userNotificationPublisher(for: .custom({ $0.userInfo["ab"] != nil }))
      .onlyResponse()
      .sink { [weak self] (response, completion) in
        if let deepLink = response.notification.deepLink {
          self?.handleLink(deepLink)
  ...
  private func configureDarkWebMonitoring() {
    notificationService.userNotificationPublisher(for: .code(.darkWebAlert)).sink {
      [weak self] event in
      switch event {
      case let .readDelivered(_, completionHandler):
...
extension UNNotification {
  fileprivate var securityToken: String? {
