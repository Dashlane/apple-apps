...
actor AccountCreationService {
    ...
    public init(sessionsContainer: SessionsContainerProtocol,
    ...
    func createAccountAndLoad(using configuration: AccountCreationConfiguration) async throws -> SessionServicesContainer {
        do {
            let session = try await createAccount(using: configuration)
            let sessionServices = try await sessionServicesLoader.load(for: session, context: .accountCreation)
    ...
    private func createAccount(using configuration: AccountCreationConfiguration) async throws -> Session {
        let sessionCryptoEngine = try sessionCryptoEngineProvider.sessionCryptoEngine(for: .masterPassword(configuration.password))
        let cryptoConfig = sessionCryptoEngine.config

                let creationInfo = try AccountCreationInfo(email: configuration.email,
        ...
        let configuration = SessionConfiguration(login: login,
                                                 ...
                                                 info: SessionInfo(deviceAccessKey: accountInfo.deviceAccessKey,
                                                                   loginOTPOption: nil,
                                                                   accountType: AccountType(configuration.accountType)))
...
extension AccountType {
    init(_ type: AccountAccountType) {
        switch type {
...
private extension SessionServicesContainer {
    func apply(_ localConfiguration: AccountCreationConfiguration.LocalConfiguration) {
...
extension AppServicesContainer {
    var accountCreationService: AccountCreationService {
        AccountCreationService(sessionsContainer: sessionContainer,
                               sessionCleaner: sessionCleaner,
                               sessionServicesLoader: sessionServicesLoader,
                               appAPIClient: appAPIClient,
                               sessionCryptoEngineProvider: sessionCryptoEngineProvider,
