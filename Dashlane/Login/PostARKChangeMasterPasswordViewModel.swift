...
class PostARKChangeMasterPasswordViewModel: ObservableObject, SessionServicesInjecting {
    ...
    init(accountCryptoChangerService: AccountCryptoChangerServiceProtocol,
    ...
    func makeMigrationProgressViewModel() -> MigrationProgressViewModel {
        let model = migrationProgressViewModelFactory.make(type: .masterPasswordToMasterPassword,
                                                           accountCryptoChangerService: accountCryptoChangerService,
                                                           context: .accountRecovery) { [weak self] result in
...
extension PostARKChangeMasterPasswordViewModel {
    static var mock: PostARKChangeMasterPasswordViewModel {
        PostARKChangeMasterPasswordViewModel(accountCryptoChangerService: AccountCryptoChangerService.mock, userDeviceAPIClient: .fake, syncedSettings: .mock, activityReporter: .fake, logger: LoggerMock(), migrationProgressViewModelFactory: .init({ _, _, _, _, _, _ in
                .mock()
        }), completion: {_ in})
    }
}
