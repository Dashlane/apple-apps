...
struct RootView: View {
    ...
    @State
    var sheetDismissAction: AuthenticationRequest? = nil
    ...
    var mainView: some View {
        ZStack {
            ...
            case .loading:
                AccountLoadingView()
            case .askForAuthentication(let model):
                UnlockView(model: model)
                    .transition(.lock)
            case let .paired(viewModel):
                PairedView(model: viewModel, showAnnouncement: $model.showAnnouncement, announcementContent: announcementView)
                    .transition(.unlock)
            case let .standalone(viewModel):
                StandaloneView(model: viewModel, showAnnouncement: $model.showAnnouncement, announcementContent: announcementView)
                    .transition(.unlock)
            }
        }
        .animation(.easeIn, value: model.appState)
        .sheet(item: $model.currentRequest, onDismiss: {
