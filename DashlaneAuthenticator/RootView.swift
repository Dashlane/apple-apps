...
struct RootView: View {
  ...
  var body: some View {
    mainView
      ...
      .onChange(of: scenePhase) { phase in
        guard let url = URL(string: "dashlane:///"), !UIApplication.shared.canOpenURL(url),
          phase == .active
        else {
          return
        }
  ...
  var mainView: some View {
    ...
    .alert(
      L10n.Localizable.authenticatorPushViewTimeOutError,
  ...
  func announcementView() -> some View {
    if let pendingRequest = model.pendingRequest.first {
      PendingAuthRequestAnnouncement(
        expiryDate: Date(timeIntervalSince1970: TimeInterval(pendingRequest.validity.expireDate))
      ) { action in
        switch action {
        case .ignore:
  ...
  func authenticationPushView(for request: AuthenticationRequest) -> AuthenticationPushView {
    AuthenticationPushView(
      ...
      model: model.makeAuthenticationPushViewModel(for: request) { action in
...
extension AnyTransition {
  ...
  fileprivate static var unlock: AnyTransition {
    return .asymmetric(
      insertion: .scale(scale: 0.7).combined(with: .opacity).animation(lockAnimation),
      removal: .opacity)
  }
}
