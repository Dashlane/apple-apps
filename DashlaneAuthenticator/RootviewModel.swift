...
class RootviewModel: ObservableObject {
  ...
  @SharedUserDefault(
    key: AuthenticatorKey.isAuthenticatorFirstLaunch,
    userDefaults: ApplicationGroup.authenticatorUserDefaults)
  ...
  func unlock(for state: PasswordAppState) {
    guard case let .locked(info) = state else {
      appservices.applicationState.move(
        to: .standAlone(StandAloneServicesContainer(appServices: appservices), state))
      return
    }
  ...
  func makeAuthenticationPushViewModel(
    for request: AuthenticationRequest,
    completion: @escaping (AuthenticationPushViewModel.Action?) -> Void
  ) -> AuthenticationPushViewModel {
    return AuthenticationPushViewModel(
      notificationService: appservices.notificationService,
      request: request, completion: completion)
  }

  func validateMasterKey(
    _ masterKey: CoreKeychain.MasterKey, for login: Login, mode: AuthenticationMode,
    ...
    let session = try appservices.sessionsContainer.loadSession(
      for: LoadSessionInformation(
        login: login, masterKey: masterKey.coreSessionMasterKey(withServerKey: serverKey)))

    return try await Task.detached(priority: .utility) {
      return try await PairedServicesContainer(
        session: session,
  ...
  func reportAppLaunch() {
    ...
    if isFirstLaunch {
      ...
      appservices.activityReporter.report(
        AnonymousEvent.OtherAuthenticatorsInstalledReport(
          otherAuthenticatorList: AnonymousEvent.OtherAuthenticatorsInstalledReport
            .other2FAappsInstalled))
    }
    appservices.activityReporter.report(
      UserEvent.AuthenticatorLaunch(
        hasPasswordManagerInstalled: hasPasswordManager, isFirstLaunch: isFirstLaunch))
  }
