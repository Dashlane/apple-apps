...
class ApplicationStateService {
  ...
  func passwordAppState() -> PasswordAppState {
    ...
    guard
      let authenticationMode = secureLockProvider.secureLockMode(checkIsBiometricSetIntact: false)
        .authenticationMode
    else {
      return .noLock
    }
    return .locked(
      SessionLoadingInfo(
        login: login, settings: settings, authenticationMode: authenticationMode,
