    ...
    init(logger: Logger, appState: ApplicationStateService) {
        ...
        messageReceiver = IPCMessageListener<PasswordAppMessage>(urlToObserve: PasswordAppCommunicator.passwordAppToAutheticatorUrl,
                                                                 coder: IPCMessageCoder(logger: logger, engine: KeychainBasedCryptoEngine.ipc(encryptionKeyId: "authenticator-encryption-id")),
                                                                    logger: logger)
        messageSender = IPCMessageSender<AutheticatorMessage>(coder: IPCMessageCoder(logger: logger, engine: KeychainBasedCryptoEngine.ipc(encryptionKeyId: "authenticator-encryption-id")),
