...
public protocol NotificationServiceProtocol {
  var remoteNotificationPublisher: CurrentValueSubject<Notification?, Never> { get }
  ...
  private func registerNotificationAction() {
    ...
    let authenticationRequestCategory =
      UNNotificationCategory(
        identifier: NotificationActionCategory.authenticationRequest.rawValue,
...
extension NotificationService: UNUserNotificationCenterDelegate {
  func userNotificationCenter(
    ...
    if UIApplication.shared.applicationState == .active {
      ...
      activityReporter.report(
        UserEvent.AuthenticatorPushAction(
          authenticatorPushStatus: .received, authenticatorPushType: .otpCode))
  ...
  public func accept(_ request: AuthenticationRequest) async throws {
    guard
      let deviceAccessKey = try? sessionsContainer.info(for: Login(request.login)).deviceAccessKey
    else {
      return
    }
    try await self.apiClient.authenticator.validateRequest(
      requestId: request.id, deviceAccessKey: deviceAccessKey,
      approval: .init(status: .approved, isSuspicious: nil))
    activityReporter.report(
      UserEvent.AuthenticatorPushAction(
        authenticatorPushStatus: .accepted, authenticatorPushType: .otpCode))
  }
  ...
  public func reject(_ request: AuthenticationRequest) async throws {
    ...
    try await self.apiClient.authenticator.validateRequest(
      requestId: request.id, deviceAccessKey: deviceAccessKey,
      approval: .init(status: .rejected, isSuspicious: false))
    activityReporter.report(
      UserEvent.AuthenticatorPushAction(
        authenticatorPushStatus: .rejected, authenticatorPushType: .otpCode))
  }
}
