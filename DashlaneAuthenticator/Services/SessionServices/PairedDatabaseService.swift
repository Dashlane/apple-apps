...
class PairedDatabaseService: AuthenticatorDatabaseServiceProtocol, SessionCredentialsProvider {
  ...
  public func update(_ item: OTPInfo) throws {
    ...
    guard var credentialHavingOTP = try? appDatabase.fetch(with: item.id, type: Credential.self)
    else {
  ...
  func load() {
    ...
    let credentials = appDatabase.itemsPublisher(for: Credential.self)
      .filter(by: userSpacesService.$configuration)
      .shareReplayLatest()
    credentials.combineLatest(databaseService.codesPublisher)
      .map {
        credentials, dashlaneOTP -> (
          codes: Set<OTPInfo>, codesAndCredentials: [OTPInfo: Identifier]
        ) in
        let credentialsAndOTPInfo: [OTPInfo: Identifier] = credentials.reduce(
          into: [OTPInfo: Identifier]()
...
extension PairedDatabaseService {
  func activityLogCreate(_ credential: Credential) {
    ...
    try? activityLogsService.report(
      .creation,
      for: info)
  }
