...
struct TokenListView<Content: View>: View {
  ...
  private var listBottomPadding: CGFloat = 80
  ...
  init(
    model: @autoclosure @escaping () -> TokenListViewModel,
      ...
      case let .detail(item):
        TokenDetailView(
          model: TokenDetailViewModel(
            token: item, databaseService: model.databaseService,
            tokenAction: handleRowTrailingAction))
      case .help:
        HelpView(addAction: addAction)
      case .sunset:
        SunsetView(model: SunsetViewModel(databaseService: model.databaseService))
      }
    }
    .navigationTitle(
      isEditing ? L10n.Localizable.tokensListStartEdit : L10n.Localizable.tokensListNavigationTitle
    )
  ...
  func rowView(for token: OTPInfo) -> some View {
    TokenRowView(
      model: model.makeTokenRowViewModel(for: token),
...
extension TokenListView {
  func alert(for item: OTPInfo) -> Alert {
    Alert(
      title: Text(L10n.Localizable.otpDeletionTitle(item.configuration.issuerOrTitle)),
      message: Text(L10n.Localizable.otpDeletionMessage(item.configuration.issuerOrTitle)),
      primaryButton: .destructive(
        Text(L10n.Localizable.otpDeletionConfirmButton), action: { model.delete(item: item) }),
  ...
  var addNewAccountButton: some View {
    Button(L10n.Localizable.addOtpFlowAddNewCta, action: { addAction(false) })
  ...
  var addNewAccountBackground: some View {
    ...
    LinearGradient(
      gradient: Gradient(colors: colors), startPoint: .init(x: 0.5, y: 0.8), endPoint: .top
  ...
  var sunsetAnnouncement: some View {
    Infobox(
      CoreLocalization.L10n.Core.authenticatorSunsetMessage("\(model.daysBeforeSunset)"),
      description: CoreLocalization.L10n.Core.authenticatorSunsetDetails
    ) {
      Button(CoreLocalization.L10n.Core.authenticatorSunsetLearnMore) {
        model.showSunset()
      }
    }
    .style(mood: .warning)
