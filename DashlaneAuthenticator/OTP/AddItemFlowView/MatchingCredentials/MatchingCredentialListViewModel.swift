...
class MatchingCredentialListViewModel: ObservableObject, AuthenticatorServicesInjecting, AuthenticatorMockInjecting {
    enum Completion {
...
extension MatchingCredentialListViewModel {
    static func mock(website: String = "facebook.com",
                     matchingCredentials: [Credential] = [PersonalDataMock.Credentials.netflix, PersonalDataMock.Credentials.adobe],
                     completion: @escaping (MatchingCredentialListViewModelCompletion) -> Void = { _ in }) -> MatchingCredentialListViewModel {
        AuthenticatorMockContainer().makeMatchingCredentialListViewModel(website: website, matchingCredentials: matchingCredentials, completion: completion)
    }
}
