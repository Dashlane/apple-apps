...
@MainActor
class AddItemScanCodeFlowViewModel: ObservableObject, AuthenticatorServicesInjecting,
  AuthenticatorMockInjecting
{
  ...
  enum Step {
    ...
    case credentialsMatchingWebsite(OTPInfo, [Credential], SessionCredentialsProvider)
  ...
  func handleOTPInfo(_ otpInfo: OTPInfo) async {
    let matchingAction = await self.mode.matchingAction(
      forWebsite: otpInfo.configuration.issuerOrTitle)
    ...
    case let .linkToCredential(credential):
      ...
      self.matchingCredentialsSaveAfterSelection(
        otpInfo: otpInfo,
        provider: provider,
    ...
    case let .showList(matchingCredentials, provider):
      self.updateStepToMatchingCredentialSelection(
        for: otpInfo, matchingCredentials: matchingCredentials, provider: provider)
    }
  }
  ...
  private func updateStepToMatchingCredentialSelection(
    for otpInfo: OTPInfo, matchingCredentials: [Credential], provider: SessionCredentialsProvider
  ) {
    let viewModel = matchingCredentialListViewModelFactory.make(
      website: otpInfo.configuration.issuerOrTitle, matchingCredentials: matchingCredentials
    ) { action in

      self.matchingCredentialsSaveAfterSelection(
        otpInfo: otpInfo, provider: provider, userAction: action, mode: .qrCode)
    }
    self.steps.append(.credentialsMatchingWebsite(otpInfo, matchingCredentials, provider))
  }

  func matchingCredentialsSaveAfterSelection(
    otpInfo: OTPInfo,
    ...
    do {
      ...
      self.steps.append(
        .preview(
          tokenRowViewModelFactory.make(token: otpInfo),
          {
...
extension AddItemScanCodeFlowViewModel {
  func makeScanQRCodeViewModel() -> ScanQRCodeViewModel {
    ScanQRCodeViewModel(logger: logger) { [weak self] otpInfo in
  ...
  func makeAddItemManuallyFlowViewModel() -> AddItemManuallyFlowViewModel {
    addManuallyViewModelFactory.make(
      mode: mode, isFirstToken: isFirstToken,
      didCreate: { [weak self] in
        self?.complete($0, mode: $1)
      })
  }

  func makeMatchingCredentialListViewModel(
    otpInfo: OTPInfo, matchingCredentials: [Credential], provider: SessionCredentialsProvider
