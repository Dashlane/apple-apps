...
class TokenDetailViewModel: ObservableObject {
  ...
  var canSave: Bool {
    !title.isEmpty && !issuer.isEmpty && !email.isEmpty && hasChanges
  }

  var hasChanges: Bool {
    title != token.configuration.title || issuer != token.configuration.issuer
  ...
  func basicTokenAction(_ action: BasicTokenRowAction) {
    switch action {
    case .copy(let code, let token):
      tokenAction(.copy(code, token: token))
    case .delete(let otp):
      tokenAction(.delete(otp))
    }
  }
}
