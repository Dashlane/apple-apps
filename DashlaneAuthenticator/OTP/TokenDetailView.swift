    ...
    VStack(spacing: 32) {
      LazyVGrid(columns: columns) {
        GeneratedOTPCodeRowView(
          ...
          performAction: model.basicTokenAction)
      }
      ...
      TokenTextField(
        label: L10n.Localizable.editTokenWebsiteLabel.uppercased(), text: $model.issuer)
      TokenTextField(label: L10n.Localizable.editTokenLoginLabel.uppercased(), text: $model.email)

      Button(
        action: { model.showAlert = true },
        label: {
          HStack {
      ...
      .alert(
        Text(L10n.Localizable.otpDeletionTitle(model.token.configuration.issuerOrTitle)),
