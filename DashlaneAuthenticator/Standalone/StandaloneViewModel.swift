...
class StandaloneViewModel: ObservableObject {
  ...
  @SharedUserDefault(
    key: AuthenticatorKey.showStandAloneOnboarding,
  ...
  func makeAddItemRootViewModel(skipIntro: Bool) -> AddItemFlowViewModel {
    services.makeAddItemFlowViewModel(
  ...
  func makeDownloadDashlaneViewModel(showStorePage: @escaping (AppStoreProductViewer) -> Void)
    ...
    -> AddItemScanCodeFlowViewModel
  {
    services.makeAddItemScanCodeFlowViewModel(
      otpInfo: otpInfo, mode: .standalone, isFirstToken: isFirstToken
    ) { [weak self] (item, _) in
