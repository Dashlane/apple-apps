...
struct StandaloneView<Content: View>: View {
  ...
  init(
    model: @autoclosure @escaping () -> StandaloneViewModel,
  ...
  var body: some View {
    ZStack {
      if model.codes.isEmpty && model.showOnboarding {
        ...
        NavigationView {
          TokenListView(
            model: model.makeTokenListViewModel(),
    ...
    .modifier(
      StandaloneViewSheetModifier(
        displayedSheet: $model.displayedSheet,
  ...
  var announcementDisplayed: some View {
    if self.showAnnouncement {
      ...
      PairingStatusAnnouncement(
        refreshPairingAnnouncement: refreshPairingAnnouncement.eraseToAnyPublisher(),
        action: { action in
          switch action {
...
private struct StandaloneViewSheetModifier: ViewModifier {
  ...
  let makeDownloadDashlaneViewModel:
    @MainActor (@escaping (AppStoreProductViewer) -> Void) -> DownloadDashlaneViewModel
        ...
        content: { sheet in
          ...
          case .downloadDashlane:
            DownloadDashlaneView(
              model: makeDownloadDashlaneViewModel { page in
...
struct StandaloneView_preview: PreviewProvider {
  static var previews: some View {
    StandaloneView(
      model: StandaloneViewModel(
        services: StandAloneServicesContainer(appServices: AppServices()),
        state: .noLogin,
