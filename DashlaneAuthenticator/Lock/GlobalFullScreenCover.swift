    ...
    if let lock = lock, let baseWindow = uiViewController.view.window {
      ...
      guard
        context.coordinator.currentLockSession == nil
          || context.coordinator.currentLockSession?.lock != lock
      else {
        return
      }
      ...
      backgroundViewController.view =
        context.coordinator.currentLockSession?.window == nil
        ? baseWindow.snapshotView(afterScreenUpdates: false)
        : context.coordinator.currentLockSession!.window.snapshotView(afterScreenUpdates: false)
      ...
      context.coordinator.currentLockSession = ModalLockSession(
        window: modalWindow,
      ...
      modalWindow.rootViewController?.present(
        lockViewController,
        animated: true)

    } else {
      guard let modalSession = context.coordinator.currentLockSession,
        let baseWindow = uiViewController.view.window
      else {
        return
      }
      modalSession.backgroundViewController.view = baseWindow.snapshotView(
...
extension View {
  ...
  func globalFullScreen<Content: View>(
    lock: Binding<Lock?>,
    @ViewBuilder content: @escaping (Lock?) -> Content
  ) -> some View {
    return self.background(
      GlobalFullScreenCoverLock(
        lock: lock,
