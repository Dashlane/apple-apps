...
class PairedViewModel: ObservableObject {
    ...
    func makeAddItemScanCodeFlowViewModel(otpInfo: OTPInfo, isFirstToken: Bool) -> AddItemScanCodeFlowViewModel {
        services.makeAddItemScanCodeFlowViewModel(otpInfo: otpInfo, mode: .paired(services.sessionCredentialsProvider), isFirstToken: isFirstToken) { [weak self] (item, mode) in
    ...
    func makeUnlockViewModel() -> UnlockViewModel {
        ...
        return services.makeUnlockViewModel(login: services.session.login,
            ...
            if masterKey.coreSessionMasterKey(withServerKey: serverKey) == self.services.session.configuration.masterKey {
