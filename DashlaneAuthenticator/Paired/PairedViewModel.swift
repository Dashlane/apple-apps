...
class PairedViewModel: ObservableObject {
    ...
    func makeAddItemRootViewModel() -> AddItemFlowViewModel {
        return services.makeAddItemFlowViewModel(hasAtLeastOneTokenStoredInVault: !codes.isEmpty,
    ...
    func makeAddItemScanCodeFlowViewModel(otpInfo: OTPInfo, isFirstToken: Bool) -> AddItemScanCodeFlowViewModel {
        services.makeAddItemScanCodeFlowViewModel(otpInfo: otpInfo, mode: .paired(services.sessionCredentialsProvider), isFirstToken: isFirstToken) { [weak self] (item, _) in
    ...
    func makeUnlockViewModel() -> UnlockViewModel {
        ...
        return services.makeUnlockViewModel(login: services.session.login,
            ...
            if self.services.session.configuration.info.loginOTPOption != .none {
                serverKey = self.services.appServices.keychainService.serverKey(for: self.services.session.login)
            }

            if masterKey.coreSessionMasterKey(withServerKey: serverKey) == self.services.session.authenticationMethod.sessionKey {
