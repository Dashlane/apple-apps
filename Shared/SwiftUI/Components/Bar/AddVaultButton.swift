...
struct AddVaultButton<Label: View>: View {
    let label: Label
    ...
    init(secureNoteState: SecureNoteState,
         ...
         onTap: @escaping () -> Void = {},
         selectAction: @escaping (VaultItem.Type) -> (),
...
extension AddVaultButton {
    ...
    private func icon(for category: ItemCategory) -> SwiftUI.Image? {
        switch category {
            case .secureNotes:
                return secureNoteState == .limited ? Image(asset: FiberAsset.imgNoteLocked) : nil
            default:
                return nil
        }
    }
...
struct NavigationBarAddIcon: View {
    var body: some View {
