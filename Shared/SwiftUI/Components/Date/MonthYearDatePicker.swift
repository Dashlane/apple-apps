...
class MonthYearPicker: UIPickerView, UIPickerViewDelegate, UIPickerViewDataSource, DatePickerComponent {
    enum Components: Int {
        case month
        case year
    }
    ...
    private func makeDate(from selectedMonthIndex: Int, and selectedYearIndex: Int) -> Date? {
        guard let baseYear = calendar.dateComponents([.year], from: minimumDate).year else { return nil }
        ...
        guard let baseYear = calendar.dateComponents([.year], from: minimumDate).year,
            let currentYear = calendar.dateComponents([.year], from: date).year else { return }
