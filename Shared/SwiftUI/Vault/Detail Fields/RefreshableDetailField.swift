...
struct RefreshableDetailField: DetailField {
    ...
    var body: some View {
        HStack {
            ...
            HStack(spacing: 25) {
                Button(action: { didTapRefresh() }, label: {
                    Image(FiberAsset.refreshButton.name)
...
struct RefreshableDetailField_Previews: PreviewProvider {
    static var previews: some View {
        MultiContextPreview {
            VStack {
                RefreshableDetailField(title: "Title", text: .constant("test"), shouldReveal: .constant(true), didTapRefresh: { })
                RefreshableDetailField(title: "Title", text: .constant("test"), shouldReveal: .constant(false), didTapRefresh: { })
