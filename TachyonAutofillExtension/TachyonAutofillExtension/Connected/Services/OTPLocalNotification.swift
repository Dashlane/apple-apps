...
final class OTPLocalNotification: LocalNotification {    
    ...
    func build() -> UNMutableNotificationContent {
        ...
        formattedPin.insert(" ", at: formattedPin.index(formattedPin.startIndex, offsetBy: formattedPin.count/2))
        
        notificationContent.title = L10n.Localizable.otpNotificationTitle(formattedPin)
        notificationContent.body = hasClipboardOverride ?
    ...
    func shouldSendNotification(previousNotification: LocalNotification?, previousNotificationDate: Date?) -> Bool {
        guard let pNotification = previousNotification, let pNotificationDate = previousNotificationDate else { return true }

        if let pOtpNotification = pNotification as? OTPLocalNotification, pOtpNotification.pin != self.pin {
            return true
        }
