...
public class LockAnimator: NSObject, UIViewControllerAnimatedTransitioning {
    ...
    public func transitionDuration(using transitionContext: UIViewControllerContextTransitioning?) -> TimeInterval {
        ...
        let cleftFrame = transitionContext.containerView.convert(cleftView.bounds, from: cleftView)
                ...
                let (topFrame, bottomFrame) = transitionContext.containerView.bounds.divided(atDistance: cleftFrame.midY, from: .minYEdge)
...
extension LockAnimator {
    ...
    private func makeFrontSnapshotView(from sourceView: UIView, byCroppingWith frame: CGRect) -> UIView {
    ...
    private func makeSlideAnimation(forFrame frame: CGRect, direction: CAAnimation.SlideDirection) -> CAAnimation {
        let containerSize = CGSize(width: frame.size.width, height: frame.size.height + 2 * Self.slideMargin)

        let anim = isOpening ? CAAnimation.slideOut(with: direction, containerSize: containerSize, shouldFade: false) : CAAnimation.slideIn(with: direction, containerSize: containerSize, shouldFade: false)

        anim.timingFunction = CAMediaTimingFunction(name: isOpening ? CAMediaTimingFunctionName.easeIn : .easeOut)
        return anim
    }
}
