...
extension AnyTransition {
  fileprivate static var bubbleAnimation: AnyTransition {
    return
      AnyTransition
...
extension View {
  fileprivate func arrowBackground() -> some View {
    self.modifier(BubbleBackgroundModifier())
  }
}

extension View {
  public func bubbleErrorMessage(text: Binding<String?>) -> some View {
    return self.overlay(
...
struct BubbleMessagePreviews: PreviewProvider {
  static var previews: some View {
    Group {
      ...
      BubbleErrorMessage(
        text: .constant(
          "The email is not validThe email is not valid.The email is not valid.The email is not valid.."
        ))
    }
    .padding()
