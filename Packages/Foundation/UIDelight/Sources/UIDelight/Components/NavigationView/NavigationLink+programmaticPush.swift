...
extension NavigationLink where Label == EmptyView {
  public init(isActive: Binding<Bool>, @ViewBuilder destination: () -> Destination) {
    self.init(destination: destination(), isActive: isActive) {
      EmptyView()
    }
  }

  public init<Tag>(tag: Tag, selection: Binding<Tag?>, @ViewBuilder destination: () -> Destination)
  where Tag: Hashable {
  ...
  public init<Item, EmbeddedDestination>(
    item: Binding<Item?>, @ViewBuilder destination: (Item) -> EmbeddedDestination
  ) where Destination == EmbeddedDestination? {
...
extension View {
  ...
  public func navigation<Tag, Destination>(
    tag: Tag, selection: Binding<Tag?>, @ViewBuilder destination: () -> Destination
  ) -> some View where Destination: View, Tag: Hashable {
    self
      .background(
        NavigationLink(
          tag: tag,
  ...
  public func navigation<Item, Destination>(
    item: Binding<Item?>, @ViewBuilder destination: (Item) -> Destination, hidden: Bool = false
