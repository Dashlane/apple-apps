...
public extension View {
        func overFullScreen<Content: View>(isPresented: Binding<Bool>,
                                       @ViewBuilder content: @escaping () -> Content) -> some View {
        let item = Binding(get: { isPresented.wrappedValue ? isPresented.wrappedValue : nil },
                           set: { isPresented.wrappedValue = $0 ?? false })

        return self.background(OverFullScreenPresenter(item: item, content: { _ in content() }))
    }

    func overFullScreen<Item, Content: View>(item: Binding<Item?>,
