...
public protocol CharonDataAPIClientEngine {
  ...
  func sendPostRequest(url: URL, body: Data, validateSuccess: Bool) async throws
...
struct CharonDataAPIClientEngineImpl: CharonDataAPIClientEngine {
  ...
  @discardableResult
  func sendPostRequest(url: URL, body: Data, validateSuccess: Bool = true) async throws
    -> CharonDataAPIClient.CharonResponse<CharonDataAPIClient.Properties.Empty>
  {
    var request = URLRequest(url: url)
  ...
  private func signAndSendRequest<T: Codable>(request: URLRequest, validateSuccess: Bool = true)
    async throws -> CharonDataAPIClient.CharonResponse<T>
  {
    var signedRequest = request
    try await signedRequest.sign(with: signer)
    let data: CharonDataAPIClient.CharonResponse<T> = try await session.charonResponse(
      from: signedRequest, using: decoder, validateSuccess: validateSuccess)
    return data
  }
...
extension URLSession {
  ...
  fileprivate func charonResponse<T: Codable>(
    from urlRequest: URLRequest, using decoder: JSONDecoder, validateSuccess: Bool = true
  ) async throws -> CharonDataAPIClient.CharonResponse<T> {
