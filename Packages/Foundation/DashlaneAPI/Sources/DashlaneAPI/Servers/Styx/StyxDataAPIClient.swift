...
public struct StyxDataAPIClient {
  ...
  public init(
    ...
    self.apiURL = apiURL
    self.engine = StyxDataAPIClientEngineImpl(
      apiURL: apiURL,
  ...
  internal init(
    apiURL: URL = Self.defaultServerURL,
...
extension AppAPIClient {
  public func makeStyxDataClient(
    apiURL: URL = StyxDataAPIClient.defaultServerURL, credentials: AppCredentials
  ) -> StyxDataAPIClient {
...
extension StyxDataAPIClient {
  ...
  public func sendEvents(_ data: Data, for logCategory: LogCategory, isTestEnvironment: Bool)
    async throws
  {
    try await engine.post(
      "event/" + logCategory.rawValue, data: data, signer: signer,
