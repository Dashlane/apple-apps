...
extension URLRequest {
    ...
    static var headersToIgnore = ["origin",
                                  "content-length",
                                  "content-encoding",
                                  "authorization",
                                  "user-agent",
        ...
        let headers = allHTTPHeaderFields?.filter { return !Self.headersToIgnore.contains($0.key.lowercased()) }  ?? [:]
        let encodedURI = url.path.components(separatedBy: "/").map(URLRequest.encode).joined(separator: "/")
        ...
        static func encode(uri: String) -> String {
                let awsURIEncodingAllowedChars = CharacterSet.alphanumerics.union(CharacterSet(charactersIn: "-._~"))
