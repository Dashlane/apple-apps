...
public final class TOTPGenerator {
  ...
  private class func code(
    ...
    switch algorithm {
    case .sha1:
      hash = CryptoKit.HMAC<Insecure.SHA1>.authenticationCode(for: counterData, using: key)
        .withUnsafeBytes(createData)
    case .sha256:
      hash = CryptoKit.HMAC<SHA256>.authenticationCode(for: counterData, using: key)
        .withUnsafeBytes(createData)
    case .sha512:
      hash = CryptoKit.HMAC<SHA512>.authenticationCode(for: counterData, using: key)
        .withUnsafeBytes(createData)
    }
