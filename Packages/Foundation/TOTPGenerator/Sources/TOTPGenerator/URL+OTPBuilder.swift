...
public extension URL {
    ...
    enum OTPURLError: Error {
        case cannotBuildURLFromItems
    }

    static func makeOTPURL(title: String,
                           login: String,
                           issuer: String?,
                           type: OTPType = .totp(period: 30),
                           secret: String,
                           digits: Int = 6,
                           algorithm: HashAlgorithm = .sha1) throws -> URL {
        var otpURL = URLComponents(string: "")!

        otpURL.scheme = "otpauth"
        otpURL.host = type.rawValue
        otpURL.path = "/\(title.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? title)"
        + ":\(login.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? login)"

        var queryItems: [URLQueryItem] = [
            URLQueryItem(name: "secret", value: secret),
            URLQueryItem(name: "issuer", value: issuer?.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed)),
