...
extension HashFunction {
  ...
  public func hash<R: ContiguousBytes>(of data: R) -> Data {
    var derivedKey = [UInt8](repeating: 0, count: digestLength)
    derivedKey.withUnsafeMutableBytes { derivedKeyBytes in
      data.withUnsafeBytes { dataBytes in
        _ = hashFunction(
          dataBytes.baseAddress, CC_LONG(dataBytes.count),
          derivedKeyBytes.baseAddress?.bindMemory(to: UInt8.self, capacity: digestLength))
      }
    }
