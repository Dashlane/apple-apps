...
extension AnonymousEvent {
  ...
  public struct `AutofillSubmitForm`: Encodable, AnonymousEventProtocol {
    ...
    public init(
      `domain`: Definition.Domain, `fieldsDetectedByAnalysisEngineCount`: Int,
      `fieldsFilledBy`: Definition.FieldsFilledByCount, `formLoadedAtDateTime`: Date,
      `formSubmittedAtDateTime`: Date, `formType`: Definition.FormType,
      `hasModifiedInitiallyAutofilledValue`: Bool, `hasPasswordField`: Bool,
      `totalFormFieldsCount`: Int
    ) {
      self.domain = domain
