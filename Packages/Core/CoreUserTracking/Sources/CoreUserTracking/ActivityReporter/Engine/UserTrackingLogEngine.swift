...
actor UserTrackingLogEngine {
    ...
    public init(installationId: LowercasedUUID,
    ...
    public func reportPageShown(_ newPage: Page, using analyticsIds: AnalyticsIdentifiers?) async {
        ...
        let userReport = Report.User(event: UserEvent.ViewPage(),
                                     installationId: self.installationId,
                                     analyticsId: analyticsIds,
                                     navigationState: browse,
                                     session: session,
                                     platform: self.platform)
        await self.store(report: userReport,
                         category: .user,
    ...
    public func report<Event: UserEventProtocol>(_ event: Event, using analyticsIds: AnalyticsIdentifiers?) async {
        ...
        let userReport = Report.User(event: event,
                                     installationId: installationId,
                                     analyticsId: analyticsIds,
                                     navigationState: browse,
                                     session: session,
                                     platform: platform)
        await store(report: userReport,
                    category: .user,
    ...
    public func report<Event: AnonymousEventProtocol>(_ event: Event) async {
        let anonymousReport = Report.Anonymous(event: event,
                                               platform: platform)
        await store(report: anonymousReport,
                    category: .anonymous,
...
private extension Definition.User {
    init?(analyticsId: AnalyticsIdentifiers?) {
        guard let analyticsId = analyticsId,
              let analyticsUserId = LowercasedUUID(uuidString: analyticsId.user) else {
                  return nil
              }
