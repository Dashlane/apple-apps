...
extension [LogCategory: Set<Data>] {
  fileprivate mutating func filtering(entries: [LogCategory: Set<Data>]) {
    for categoryAndData in self {
      let filtered = (self[categoryAndData.key] ?? []).subtracting(
        entries[categoryAndData.key] ?? [])
      self[categoryAndData.key] = filtered
    }
  }

  fileprivate mutating func filtering(entries: Set<Data>) {
    for categoryAndData in self {
