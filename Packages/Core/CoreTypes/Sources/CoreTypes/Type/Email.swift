...
public struct Email: ExpressibleByStringLiteral, CustomStringConvertible, Hashable, Sendable {
  static let regex = try! NSRegularExpression(
    pattern: "[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\\" + "_" + "(?:[a-z0-9-]*[a-z0-9])?\\.)+[a-z0-9]"
  ...
  public init(_ stringValue: String) {
    ...
    if let start = addressStartIndex, let end = addressEndIndex, start < end {
      ...
      if start > stringValue.startIndex {
        displayName = String(stringValue[...stringValue.index(before: start)]).trimmingCharacters(
