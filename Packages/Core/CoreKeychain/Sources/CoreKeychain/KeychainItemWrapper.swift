...
public class KeychainItemWrapper {
  ...
  private func dictToSecItemData(dict: [String: AnyObject]) -> [String: AnyObject] {
    ...
    do {
      ...
      returnDict[kSecValueData as String] =
        passwordString.data(using: String.Encoding.utf8.rawValue) as AnyObject
    } catch let error as NSError {
  ...
  private func writeKeychain() {
    ...
    let copyMatchingResult = SecItemCopyMatching(
      self.genericPasswordQuery as CFDictionary, &attributes)
