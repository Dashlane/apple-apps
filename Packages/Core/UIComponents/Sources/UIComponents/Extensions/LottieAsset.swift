...
public extension LottieAsset {
    func animation(for scheme: ColorScheme = .light) -> LottieAnimation! {
        guard let base = bundle.resourceURL else {
            return nil
        }

        if scheme == .dark {
            return LottieAnimation.filepath(base.appendingPathComponent(darkAppearanceFile).path, animationCache: DefaultAnimationCache.sharedCache)
        } else {
            return LottieAnimation.filepath(base.appendingPathComponent(lightAppearanceFile).path, animationCache: DefaultAnimationCache.sharedCache)
        }
    }
...
public extension DefaultAnimationCache {
    func load(_ assets: [LottieAsset]) async {
