...
final public class Settings {
  ...
  public func set<T: DataConvertible>(value: T?, forIdentifier identifier: String) {
    ...
    guard value == nil || metadata.type == T.self else {
      preconditionFailure(
        "Trying to set a setting of type \(metadata.type.self) with a value of type \(T.self)")
    }
    let context = dataStack.currentThreadContext
    context.performAndWait {
      do {
        ...
        let kvp =
          results?.first ?? NSEntityDescription.insertNewObject(
            forEntityName: "KeyValuePair", into: context) as! KeyValuePair
        kvp.key = identifier
        if metadata.secure {
