...
extension SharingUpdater {
  func deleteItemGroupsWithoutCurrentUser(from itemGroups: [ItemGroup]) async throws {
    ...
    logger.debug(
      "\(groupsToDelete.count) ItemGroup(s) cleaned that does not contain current user, items within the group(s) removed"
    )
  ...
  func deleteItemGroupsWithCurrentUserAloneAdmin(
    from itemGroups: [ItemGroup], nextRequest: inout UpdateRequest
  ) async throws {
    let groupsToDelete = itemGroups.filter { group in
      guard let user = group.user(with: userId),
        ...
        group.userGroupMembers.filter({ $0.status.isAcceptedOrPending }).isEmpty,
        group.collectionMembers.filter({ $0.status.isAcceptedOrPending }).isEmpty
      else {
        return false
      }
      return true
    ...
    for group in groupsToDelete {
      do {
        _ = try await sharingClientAPI.deleteItemGroup(
    ...
    logger.debug(
      "\(successfullDeletedIds.count) ItemGroup(s) cleaned that contain only current user, item(s) recreated on vault"
    )
  }

  func autoRevokeUsersWithInvalidProposeSignature(
    in itemGroups: [ItemGroup], nextRequest: inout UpdateRequest
  ) async throws {
    ...
    for itemGroup in itemGroups {
      guard let groupKey = try groupKeyProvider.groupKey(for: itemGroup),
        let state = try database.sharingMembers(forUserId: userId, in: itemGroup)
          .computeItemState(),
        state.isAccepted == true, state.permission == .admin
      else {
        continue
      }
      ...
      logger.debug(
        "\(userIdsToRevoke.count) user(s) will be automatically revoked from ItemGroup \(itemGroup.info.id) due to invalid propose signature"
      )

      nextRequest += try await sharingClientAPI.revokeOnItemGroup(
        withId: itemGroup.id,
