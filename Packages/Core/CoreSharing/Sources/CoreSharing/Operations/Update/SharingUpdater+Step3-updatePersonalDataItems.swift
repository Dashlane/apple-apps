...
extension SharingUpdater {
  ...
  func updatePersonalDataItems(for request: PersonalDataUpdateRequest, allItemGroups: [ItemGroup])
    async throws
  {
    ...
    let groupsForUpdatedItemContents = allItemGroups.filter(
      forItemIds: Set(insertedOrUpdatedItemContents.keys))
  ...
  private func updates(
    for group: ItemGroup, insertedOrUpdatedItemContents: [Identifier: ItemContentCache]
  ) -> [SharingItemUpdate] {
    do {
  ...
  private func makeItemUpdate(
    itemKeyPair: ItemKeyPair,
    itemState: SharingItemUpdate.State,
    itemContent: ItemContentCache?,
    groupKey: SharingSymmetricKey<ItemGroup>
  ) throws -> SharingItemUpdate {
    let transactionContent: Data?
    if let itemContent = itemContent {
      let itemKey = try itemKeyPair.key(using: groupKey, cryptoProvider: cryptoProvider)
      transactionContent = try itemContent.content(using: itemKey, cryptoProvider: cryptoProvider)
    } else {
      transactionContent = nil
    }

    return SharingItemUpdate(
      id: itemKeyPair.id,
      state: itemState,
