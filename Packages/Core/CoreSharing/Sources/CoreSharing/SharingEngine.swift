...
public struct SharingEngine<UIDatabase: SharingUIDatabase> {
  ...
  func execute(
    maxIteration: Int = 5,
    _ action: @escaping @SharingActor (_ nextRequest: inout SharingUpdater.UpdateRequest)
      async throws -> Void
  ) async throws {
  ...
  public func getTeamLogins() async throws -> [String] {
    return try await sharingClientAPI.getTeamLogins()
  }
}
...
extension SharingUpdater {
  func retrieveMissingItemsInPersonalData() async throws {
    ...
    try await updatePersonalDataItems(
      for: updateRequest,
      allItemGroups: try database.fetchAllItemGroups()
    )
  }
}

public enum ItemDeleteBehaviour: Sendable {
  case normal
