...
extension UserGroup: FetchableRecord {
    static let request = UserGroupInfo
...
extension UserGroupInfo: TableRecord, FetchableRecord, PersistableRecord {
    ...
    static let acceptedOrPendingItemGroups = hasMany(ItemGroupInfo.self, through: acceptedOrPendingGroupMembers, using: UserGroupMember.parentItemGroup)
    static let acceptedOrPendingGroupMembers = hasMany(UserGroupMember.self, using: ForeignKey([Column.id])).filter(status: [.accepted, .pending])
}
