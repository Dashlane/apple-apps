...
struct ObfuscatedFieldInputView: View {
  ...
  init(label: String, text: Binding<String>, placeholder: String?, isPassword: Bool) {
    self.label = label
    self.text = text
  ...
  var body: some View {
    ...
    .onChange(of: isSecureValueRevealed) { _, newValue in
  ...
  private var plainPasswordField: some View {
    TextField("", text: text)
  ...
  private var plainObfuscatedField: some View {
    TextField("", text: text)
      .foregroundStyle(.textInputValue)
      .textStyle(
        text.wrappedValue.isEmpty
