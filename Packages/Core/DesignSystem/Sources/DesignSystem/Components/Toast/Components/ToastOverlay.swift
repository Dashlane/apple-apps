...
extension AnyTransition {
  fileprivate static var insertToast: AnyTransition {
    .move(edge: .top).combined(with: .opacity)
  }

  fileprivate static var removeToast: AnyTransition {
    .modifier(
      active: DisappearModifier(blurRadius: 13, opacity: 0, scale: 0.5),
