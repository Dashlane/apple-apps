...
struct ColorGenerator {
  ...
  private func fetchColors(for userInterfaceStyle: UserInterfaceStyle) async throws
    -> [SpecifyColor]
  {
  ...
  private func generateAssetCatalogue(with colors: [ColorAsset]) throws {
    let assetCatalogueURL = destination.appendingPathComponent(
      "Resources/Assets.xcassets", isDirectory: true)
  ...
  private func includeFoldersToColorNames() throws {
    let colorsAssetPath = destination.appendingPathComponent(
      "Resources/Assets.xcassets/Colors", isDirectory: true)
    let subPaths = try FileManager.default.subpathsOfDirectory(atPath: colorsAssetPath.relativePath)
    let encoder = JSONEncoder.sortedPrettyPrinted
    for subPath in subPaths {
      guard
        !subPath.hasSuffix(".colorset") && !subPath.hasSuffix(".json") && !subPath.hasPrefix(".")
      else { continue }
...
extension Dictionary where Key == String, Value == RGBAValue {
  fileprivate func merged(with darkColorDictionary: [String: RGBAValue]) throws -> [String: (
    light: RGBAValue, dark: RGBAValue
  )] {
  ...
  fileprivate func differences(_ dictionary: Dictionary) -> Set<Dictionary.Key> {
    return Set(keys).symmetricDifference(Set(dictionary.keys))
  }
}

extension URL {
  fileprivate var subfolders: [URL] {
    return
      (try? FileManager.default.contentsOfDirectory(
        at: self, includingPropertiesForKeys: nil, options: [])) ?? [URL]()
  }
}

extension Array where Element == SpecifyColor {
  fileprivate func dictionary() -> [String: RGBAValue] {
    return self.reduce(into: [String: RGBAValue]()) { partialResult, color in
      partialResult[color.name] = color.rgbaValue
    }
  }

  fileprivate func paired(with darkModeColors: [SpecifyColor]) throws -> [ColorAsset] {
    ...
    let colorAssets = mergedDict.map { colorName, colorValues -> ColorAsset in
      ...
      let normalizedName =
        nameComponents.first! + nameComponents.dropFirst().map(\.capitalized).joined()
