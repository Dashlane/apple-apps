...
struct IconGenerator {
  ...
  func generate() async throws {
    ...
    let decodedIcons = try JSONDecoder().decode([SpecifyIcon].self, from: data).filter(
      \.shouldBeProcessed)
  ...
  private func deleteIconsAssetCatalogue() throws {
    let iconsDirectory = destination.appendingPathComponent(
      "Resources/Assets.xcassets/Icons/", isDirectory: true)
  ...
  private func generateAssetCatalogue(for icons: [SpecifyIcon]) throws {
    ...
    for icon in icons {
      ...
      try FileManager.default.createDirectory(
        at: completeDirectory, withIntermediateDirectories: true, attributes: nil)
    ...
    for subdirectoryPath in paths {
      ...
      while !pathComponents.isEmpty {
        ...
        let subdirectory = assetCatalogueURL.appendingPathComponent(
          "Icons/\(pathComponent)", isDirectory: true)
