...
public struct Categorizer: CategorizerProtocol {
  ...
  public init() throws {
    ...
    categories = try db.query(statement: SQLQuery.getAllCategories.buildQuery()).rows.compactMap(
      Category.init)
  ...
  public func getAllDomains() throws -> [String] {
    try db.query(statement: SQLQuery.getAllDomains.buildQuery()).rows.compactMap(Domain.init).map(
      \.domain)
  }
}
