import CoreLocalization
import Foundation
import UIKit

public struct ShortcutAction: Hashable {
  public let shortcut: DynamicShortcut
  public let action: () -> Void

  public func hash(into hasher: inout Hasher) {
    hasher.combine(shortcut)
  }

  public static func == (lhs: ShortcutAction, rhs: ShortcutAction) -> Bool {
    lhs.shortcut == rhs.shortcut
  }
}

public enum ShortcutMenu {
  case edit
  case view
  case application

  public init(menuIdentifier: UIMenu.Identifier) {
    switch menuIdentifier {
    case .edit: self = .edit
    case .view: self = .view
    case .application: self = .application
    default: self = .view
    }
  }

  public var identifier: UIMenu.Identifier {
    switch self {
    case .edit: return .edit
    case .view: return .view
    case .application: return .application
    }
  }
}

public struct DynamicShortcut: Hashable {
  public let title: String
  public let input: String
  public let modifier: UIKeyModifierFlags
  public let triggerIdentifier: String
  public let menu: ShortcutMenu
  public let id = UUID().uuidString

  public init(title: String, input: String, modifier: UIKeyModifierFlags, menu: ShortcutMenu) {
    self.title = title
    self.input = input
    self.modifier = modifier
    self.triggerIdentifier = "\(input)-\(modifier.rawValue)"
    self.menu = menu
  }

  public func command(selector: Selector) -> UIKeyCommand {
    UIKeyCommand(
      title: title,
      image: nil,
      action: selector,
      input: input,
      modifierFlags: modifier,
      propertyList: [triggerIdentifier: title])
  }

  public func hash(into hasher: inout Hasher) {
    hasher.combine(triggerIdentifier)
  }

  public static func == (lhs: DynamicShortcut, rhs: DynamicShortcut) -> Bool {
    lhs.triggerIdentifier == rhs.triggerIdentifier
  }
}

extension DynamicShortcut {
  public static var search: DynamicShortcut {
    .init(
      title: CoreL10n.keyboardShortcutSearch,
      input: "F",
      modifier: .command,
      menu: .view)
  }

  public static func copyPrimary(title: String) -> DynamicShortcut {
    .init(
      title: title,
      input: "C",
      modifier: .command,
      menu: .edit)
  }

  public static func copySecondary(title: String) -> DynamicShortcut {
    .init(
      title: title,
      input: "C",
      modifier: [.command, .shift],
      menu: .edit)
  }

  public static var delete: DynamicShortcut {
    .init(
      title: CoreL10n.kwDelete,
      input: "\u{8}",
      modifier: .command,
      menu: .edit)
  }

  public static var edit: DynamicShortcut {
    .init(
      title: CoreL10n.kwEdit,
      input: "E",
      modifier: .command,
      menu: .edit)
  }

  public static var save: DynamicShortcut {
    .init(
      title: CoreL10n.kwSave,
      input: "S",
      modifier: .command,
      menu: .edit)
  }

  public static var cancel: DynamicShortcut {
    .init(
      title: CoreL10n.cancel,
      input: "\u{1B}",
      modifier: .command,
      menu: .edit)
  }

  public static var back: DynamicShortcut {
    .init(
      title: CoreL10n.kwBack,
      input: "\u{1B}",
      modifier: .command,
      menu: .view)
  }

  public static var preferences: DynamicShortcut {
    .init(
      title: CoreL10n.kwSettings,
      input: ",",
      modifier: .command,
      menu: .application)
  }
}
