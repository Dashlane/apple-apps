...
struct OnDiskRemoteFileCache {
  ...
  init(cacheDirectory: URL, cryptoEngine: CryptoEngine, logger: Logger) {
    ...
    do {
      try FileManager.default.createDirectory(
        at: cacheDirectory, withIntermediateDirectories: true, attributes: [:])
    } catch {
  ...
  func cache(forKey key: String) throws -> (data: Data, modificationDate: Date)? {
    ...
    let plaintext = try cryptoEngine.decrypt(data)
  ...
  func etag(forKey key: String) throws -> String? {
    let url = try localCacheURL(for: key).appendingPathExtension(
      OnDiskRemoteFileCache.etagFilenameSuffix)
