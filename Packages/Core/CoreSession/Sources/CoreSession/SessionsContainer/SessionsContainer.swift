  ...
  SessionsContainerProtocol
{
  ...
  public init(
    ...
    sessionStoreProvider: StoreProvider
  ) throws {
    guard baseURL.isFileURL else {
      throw SessionsContainerError.invalidURL("Session URL needs to be local")
    }
  ...
  private func writeSession(
    with configuration: SessionConfiguration, localKey: Data, cryptoConfig: CryptoRawConfig
  ) throws -> Session {
    let login = configuration.login
    let cryptoEngine = try cryptoEngineProvider.sessionCryptoEngine(
      for: cryptoConfig, masterKey: configuration.masterKey)
    let encryptedLocalKey = try localKey.encrypt(using: cryptoEngine)

    let localCryptoEngine = try cryptoEngineProvider.cryptoEngine(forKey: localKey)
    let remoteCryptoEngine =
      try configuration.keys.remoteKey.map(cryptoEngineProvider.cryptoEngine) ?? cryptoEngine
    ...
    try sessionStoreProvider.encryptedLocalKeyStore(
      for: login, info: configuration.info, directory: directory
    ).save(encryptedLocalKey)
    try sessionStoreProvider.infoStore(for: login, directory: directory)
      .save(configuration.info)
    try sessionStoreProvider.keysStore(
      for: login, directory: directory,
      using: .init(session: cryptoEngine, local: localCryptoEngine), info: configuration.info
  ...
  public func update(_ cryptoConfig: CryptoRawConfig, for session: Session) throws {
    ...
    try sessionStoreProvider.encryptedLocalKeyStore(
      for: session.login, info: session.configuration.info, directory: session.directory
    ).save(encryptedLocalKey)
  }
  ...
  public func loadSession(for loadInfo: LoadSessionInformation) throws -> Session {
    ...
    let encryptedLocalKey = try sessionStoreProvider.encryptedLocalKeyStore(
      for: login, info: info, directory: directory
    ).load()
    let cryptoEngine = try cryptoEngineProvider.sessionCryptoEngine(
      forEncryptedPayload: encryptedLocalKey, masterKey: loadInfo.masterKey)
    ...
    let localCryptoEngine = try cryptoEngineProvider.cryptoEngine(forKey: localKey)
    let keys = try sessionStoreProvider.keysStore(
      for: login, directory: directory,
      using: .init(session: cryptoEngine, local: localCryptoEngine), info: info
    ).load()

    let remoteCryptoEngine =
      try keys.remoteKey.map(cryptoEngineProvider.cryptoEngine) ?? cryptoEngine
...
extension SessionsContainer {
  ...
  public func prepareMigration(
    ...
    let cryptoEngine = try cryptoEngineProvider.sessionCryptoEngine(
      for: cryptoConfig, masterKey: newConfiguration.masterKey)
    let remoteCryptoEngine =
      try newConfiguration.keys.remoteKey.map(cryptoEngineProvider.cryptoEngine) ?? cryptoEngine

    return MigratingSession(
  ...
  public func finalizeMigration(using migrationSession: MigratingSession) throws -> Session {
    let newSession = try writeSession(
      with: migrationSession.target.configuration, localKey: migrationSession.source.localKey,
      cryptoConfig: migrationSession.target.cryptoConfig)
  ...
  public func localMigration(
    ...
    let keys = SessionSecureKeys(
      serverAuthentication: session.configuration.keys.serverAuthentication,
      remoteKey: remoteKey,
      analyticsIds: session.configuration.keys.analyticsIds)

    let sessionConfiguration = SessionConfiguration(
      login: session.login, masterKey: .ssoKey(ssoKey), keys: keys, info: session.configuration.info
    )

    return try writeSession(
      with: sessionConfiguration, localKey: session.localKey, cryptoConfig: config)
  }
