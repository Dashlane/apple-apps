...
public extension KeyedStore {
    func store(_ object: String, for key: Key) throws {
        let data = object.data(using: .utf8)
        try store(data, for: key)
    }

    func retrieveString(for key: Key) throws -> String {
...
public extension KeyedStore {
    func store<T: FixedWidthInteger>(_ object: T, for key: Key) throws {
        let data = withUnsafeBytes(of: object.littleEndian) { Data($0) }
        try store(data, for: key)
    }

    func retrieve<T: FixedWidthInteger>(for key: Key) throws -> T {
        let data = try retrieveData(for: key)
...
public extension KeyedStore {
    func store<T: Encodable>(_ object: T?, for key: Key) throws {
...
public extension KeyedStore {
    func store(_ bool: Bool, for key: Key) throws {
        let data = Data([UInt8(bool ? 1 : 0)])
        try store(data, for: key)
    }

    func retrieve(for key: Key) throws -> Bool {
        let data = try retrieveData(for: key)
