...
public struct SessionStoreProvider: SessionStoreProviderProtocol {
    ...
    public func infoStore(for login: Login, directory: SessionDirectory) throws -> InfoStore {
        InfoStore(url: URL(baseURL: directory.url.appendingPathComponent(Keys.info.rawValue), key: .info))
    }

    public func encryptedLocalKeyStore(for login: Login, info: SessionInfo, directory: SessionDirectory) throws -> LocalKeyStore {
        LocalKeyStore(url: URL(baseURL: directory.url.appendingPathComponent(Keys.localKey.rawValue), key: .localKey))
    }

    public func keysStore(for login: Login, directory: SessionDirectory, using engineSet: StoreCryptoEngineSet, info: SessionInfo) throws -> KeysStore {
        KeysStore(url: URL(baseURL: directory.url.appendingPathComponent(Keys.keys.rawValue), key: .keys), cryptoEngine: engineSet.local)
    }
}
