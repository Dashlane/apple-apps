...
public enum UnlockType {
    case mpValidation
    case mpOtp2Validation(authTicket: String?, serverKey: String) 
    case ssoValidation(_ validator: SSOLocalLoginValidator, authTicket: String? = nil, remoteKey: Data? = nil)
...
extension UnlockType {
    @MainActor func localLoginStep(with handler: LocalLoginHandler, session: Session, context: LoginContext, cryptoEngineProvider: CryptoEngineProvider) -> LocalLoginHandler.Step {
        switch self {
    ...
    @MainActor private func migrationstep(with handler: LocalLoginHandler, session: Session, authTicket: String?, context: LoginContext, cryptoEngineProvider: CryptoEngineProvider) -> LocalLoginHandler.Step? {
        if let ssoMigration = handler.ssoInfo,
           ...
           let serviceProviderUrl = URL(string: "\(ssoMigration.serviceProviderUrl)?redirect=\(context.origin.rawValue)&username=\(handler.login.email)&frag=true") {
            let validator = SSOLocalLoginValidator(login: handler.login,
                                                   ...
                                                   accountAPIClient: handler.accountAPIClient,
