...
final class SQLiteInterProcessCommunicator: NSObject {
    ...
    private static func makeNotificationName(identifier: SQLiteClientIdentifier, action: Action) -> CFString {
    ...
    deinit {
        ...
        for action in Action.allCases {
            CFNotificationCenterAddObserver(center, Unmanaged.passRetained(self).toOpaque(), { (_, _, name, _, _) in
            ...
            }, Self.makeNotificationName(identifier: identifier, action: action), nil, .deliverImmediately)
        }
    }
    ...
    func post(_ action: Action) {
        for identifier in SQLiteClientIdentifier.allCases where identifier != self.identifier {
            ...
            CFNotificationCenterPostNotification(CFNotificationCenterGetDarwinNotifyCenter(), CFNotificationName(rawValue: notification), nil, nil, true)
        }
    }
}
