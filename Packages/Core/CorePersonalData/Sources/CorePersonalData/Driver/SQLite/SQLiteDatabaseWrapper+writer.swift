...
extension SQLiteDatabaseWrapper: DatabaseWriter {
    ...
    public mutating func updateMetadata(forSyncRequestId syncRequestId: String, shouldCreateSnapshot: Bool, using updater: (inout RecordMetadata) -> Void) throws {
        guard let rawRecord = try RawRecord.filter(MetadataColumns.syncRequestId == syncRequestId).fetchOne(db) else {
           return
        }
