...
public struct PersonalDataDecoder {
  ...
  public func decode<T>(
    _ type: T.Type, from value: PersonalDataValue, using linkedFetcher: LinkedFetcher? = nil
  ...
  public func decode<T>(
    _ type: T.Type, from record: PersonalDataRecord, using linkedFetcher: LinkedFetcher? = nil
  ) throws -> T where T: Decodable {
    let options = Options(
      userInfo: [:],
      codeDecoder: codeDecoder,
...
struct PersonalDataDecoderImpl: Decoder {
  ...
  init(
    value: PersonalDataValue?, codingPath: [CodingKey] = [], options: PersonalDataDecoder.Options
  ) {
    self.value = value
  ...
  where Key: CodingKey {
    ...
    case let .collection(collection):
      let container = KeyedDecodeContainer<Key>(
    ...
    case nil:
      throw DecodingError.valueNotFound(
        PersonalDataCollection.self,
        makeErrorContext(
          description:
            "Expected to decode \(PersonalDataCollection.self) but found no value instead."))
    default:
      throw DecodingError.typeMismatch(
        PersonalDataCollection.self,
        makeErrorContext(
          description:
            "Expected to decode \(PersonalDataCollection.self) but found \(value.debugCoderDescription) instead."
        ))
    }
  ...
  func unkeyedContainer() throws -> UnkeyedDecodingContainer {
    ...
    guard case let .list(list) = value else {
      throw DecodingError.typeMismatch(
        PersonalDataList.self,
        makeErrorContext(
          description:
            "Expected to decode \(PersonalDataList.self) but found \(value.debugCoderDescription) instead."
        ))
    }

    return UnkeyedDecodeContainer(
      list: list,
      codingPath: codingPath,
      options: options)
  }
  ...
  func singleValueContainer() throws -> SingleValueDecodingContainer {
    return SingleValueDecodeContainer(
      decoder: self,
      codingPath: codingPath,
      options: options)
  }
...
extension PersonalDataDecoderImpl {
  func unwrapString() throws -> String {
    ...
    guard case let .item(string) = value else {
      throw DecodingError.typeMismatch(
        ...
        makeErrorContext(
          description:
            "Expected to decode \(String.self) but found \(value.debugCoderDescription) instead."))
    }
  ...
  func unwrapDate() throws -> Date {
    ...
    guard let seconds = TimeInterval(value) else {
      throw DecodingError.dataCorrupted(
        makeErrorContext(description: "Invalid string \(value) for type \(Date.self)."))
    }
  ...
  func unwrapURL() throws -> URL {
    ...
    guard let url = URL(string: value) else {
      throw DecodingError.dataCorrupted(
        makeErrorContext(description: "Invalid string for type \(URL.self)."))
    }
  ...
  func unwrapData() throws -> Data {
    ...
    guard !value.isEmpty, let data = Data(base64Encoded: value) else {
      throw DecodingError.dataCorrupted(
        makeErrorContext(description: "Invalid string for type \(Data.self)."))
    }
  ...
  func unwrapCodeNamePair(using type: CodeNamePair.Type) throws -> CodeNamePair {
    ...
    guard let decoder = options.codeDecoder,
      let name = try decoder.decodeCode(value, for: type.codeFormat)
    else {
  ...
  func unwrap<T: Decodable>(as type: T.Type) throws -> T {
    if type == Date.self {
      ...
      guard case let .object(object) = value, object.type == nestedType.contentType else {
        throw DecodingError.typeMismatch(
          PersonalDataObject.self,
          makeErrorContext(
            description:
              "Expected to decode \(PersonalDataObject.self) with content type \(nestedType.contentType) but found \(value.debugCoderDescription) instead."
          ))
      }
...
extension PersonalDataValue {
  fileprivate var debugCoderDescription: String {
    switch self {
    case .list:
...
extension Optional where Wrapped == PersonalDataValue {
  fileprivate var debugCoderDescription: String {
    switch self {
    case let .some(value):
