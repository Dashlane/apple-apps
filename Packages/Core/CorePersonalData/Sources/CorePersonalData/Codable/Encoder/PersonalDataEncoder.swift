...
public struct PersonalDataEncoder {
  ...
  public func encode<T: Encodable>(_ value: T, in collection: PersonalDataCollection = [:]) throws
    -> PersonalDataCollection
  {
    ...
    guard let value = ref.personalDataValue()?.collection else {
      throw DecodingError.valueNotFound(
        T.self, .init(codingPath: [], debugDescription: "Cannot decode \(T.self)"))
    }
    return value
  }
}
...
struct PersonalDataEncoderImpl: Encoder {
  ...
  func container<Key>(keyedBy type: Key.Type) -> KeyedEncodingContainer<Key> where Key: CodingKey {
    ...
    let container = KeyedEncodeContainer<Key>(
      encoder: self, collection: collection, codingPath: self.codingPath)
