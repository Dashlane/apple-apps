...
public class SharingPersonalDataDBStack {
  ...
  public init(
    ...
    self.decoder = PersonalDataDecoder(
      codeDecoder: codeDecoder, personalDataURLDecoder: personalDataURLDecoder)
  }
}
...
extension SharingPersonalDataDBStack: SharingPersonalDataDB {
  ...
  private func performFullUpdate(
    _ update: SharingItemUpdate, content: Data, in db: inout DatabaseWriter
  ) throws {
  ...
  private func performSharingStateUpdateOnly(
    _ update: SharingItemUpdate, in db: inout DatabaseWriter
  ) throws {
    if update.state.isAccepted {
  ...
  public func pendingUploads() async throws -> [SharingItemUpload] {
    ...
    return try await withThrowingTaskGroup(of: SharingItemUpload.self) { group in
      for record in records {
        guard let uploadId = record.metadata.pendingSharingUploadId,
          record.metadata.syncStatus != .pendingRemove
        else {
          continue
        }
    ...
    $pendingActivationRecords.map { records in
      return records.compactMapValues { [weak self] record in
        ...
        return try? self.decoder.decode(
          sharingType, from: record, using: LinkedFetcherImpl(driver: self.driver))
      }
    }.eraseToAnyPublisher()
  }
...
extension RecordMetadata {
  fileprivate mutating func apply(_ state: SharingItemUpdate.State) {
    isShared = true
...
extension PersonalDataRecord {
  fileprivate func makeSharingMetadata() -> SharingMetadata? {
  ...
  fileprivate func makeCreateSharingContent() throws -> SharingCreateContent? {
    guard let metadata = makeSharingMetadata() else {
      return nil
    }
    let content = try compressedXMLData()

    return SharingCreateContent(
      id: id,
      metadata: metadata,
