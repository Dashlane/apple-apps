...
public struct SyncDBStack: SyncableDatabase {
    ...
    public func parse(fromEncryptedBackupData data: Data, timestamps: TimestampByIds) throws -> [PersonalDataRecord] {
        return try Array(compressedBackupXMLData: data.decrypt(using: transactionCryptoEngine))
            .compactMap { record in
                guard let timestamp = timestamps[record.id] else {
                    return nil
                }
                var record = record
                record.metadata.lastSyncTimestamp = timestamp
