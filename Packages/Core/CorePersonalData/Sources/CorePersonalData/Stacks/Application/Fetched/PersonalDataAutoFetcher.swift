...
class PersonalDataAutoFetcher<T: PersonalDataCodable> {
  ...
  private func update(for changeSets: Set<DatabaseChange>) throws {
    ...
    logger.debug(
      "Updating items for type \(T.contentType) insertOrUpdatedIds: \(insertedOrUpdatedIds) removedIds: \(removedIds)"
    )
    itemsPublisher.send(self.items)
  }
}

extension Dictionary where Key == Identifier, Value: PersonalDataCodable {
  fileprivate init(_ items: [Value]) {
