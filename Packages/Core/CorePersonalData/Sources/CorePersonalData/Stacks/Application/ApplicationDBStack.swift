...
public struct ApplicationDBStack: ApplicationDatabase {
  ...
  public init(
    ...
    self.decoder = PersonalDataDecoder(
      codeDecoder: codeDecoder,
      personalDataURLDecoder: personalDataURLDecoder)
  }

  @inline(__always)
  private func decodeIgnoringErrors<Output: PersonalDataCodable>(
    _ output: Output.Type, from records: [PersonalDataRecord]
  ) -> [Output] {
  ...
  @inline(__always)
  func decode<Output: PersonalDataCodable>(
    _ output: Output.Type, from records: [PersonalDataRecord], ignoreDecodingErrors: Bool
  ) throws -> [Output] {
  ...
  @inline(__always)
  func decode<Output: PersonalDataCodable>(_ output: Output.Type, from record: PersonalDataRecord)
    throws -> Output
  {
  ...
  @inline(__always)
  func decode<Output: Decodable>(_ output: Output.Type, from collection: PersonalDataCollection)
    throws -> Output
  {
    try decoder.decode(Output.self, from: .collection(collection), using: makeLinkedFetcher())
  }
...
extension ApplicationDatabase where Self == ApplicationDBStack {
  public static func mock(
    items: [PersonalDataCodable] = [], driver: DatabaseDriver = InMemoryDatabaseDriver()
  ) -> ApplicationDatabase {
    let database = ApplicationDBStack(
      driver: driver,
      historyUserInfo: HistoryUserInfo(
