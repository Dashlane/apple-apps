...
public struct LostOTPSheetModifier: ViewModifier {
    ...
    public init(isLostOTPSheetDisplayed: Binding<Bool>,
    ...
    public func body(content: Content) -> some View {
        content
            ...
            .actionSheet(isPresented: $isLostOTPSheetDisplayed,
                         content: { actionSheet })
            ...
            .modifier(RecoveryTextfieldAlertModifier(item: $lostOTPSheetViewModel.textfieldAlertItem, inputBackupCode: $inputBackupCode, context: context, useBackupCode: useBackupCode))
    }

        private var actionSheet: ActionSheet {
        ActionSheet(title: .init(context.title),
                    message: .init(L10n.Core.otpRecoveryCannotAccessCodesDescription),
                    buttons: [
                        ActionSheet.Button.default(.init(context.sheetCta), action: {
                            lostOTPSheetViewModel.textfieldAlertItem = .recoveryCode
                        }),
                        ActionSheet.Button.default(.init(L10n.Core.otpRecoveryReset2Fa), action: {
        ...
        Alert(title: .init(L10n.Core.otpRecoverySendFallbackSmsTitle),
              message: .init(context.smsMessage),
              primaryButton: .default(.init(L10n.Core.kwSend),
    ...
    private var recoveryCodesConfirmationAlert: Alert {
        ...
        case .noValidPhoneNumber:
            return Alert(title: .init(L10n.Core.kwErrorTitle),
                         message: .init(L10n.Core.otpRecoverySendFallbackSmsNoPhoneNumber),
                         dismissButton: .default(.init(L10n.Core.kwButtonOk),
...
struct RecoveryTextfieldAlertModifier: ViewModifier {
    ...
    func body(content: Content) -> some View {
        if let item = item {
          content
                .modifier(AlertTextFieldModifier(item: $item,
