      ...
      .overlay(alignment: .bottom) {
        VStack(spacing: 10) {
          if model.step == .masterPasswordCreation {
            Button(L10n.Core.createMasterPasswordAccountCta) {
              model.next()
            }
        ...
        feedback: {
          if !model.password.isEmpty && model.password == model.confirmationPassword {
            FieldTextualFeedback(
              L10n.Core.minimalisticOnboardingMasterPasswordConfirmationPasswordsMatching)
          }
  ...
  extension NewMasterPasswordView where Accessory == EmptyView {
    public init(model: NewMasterPasswordViewModel, title: String) {
