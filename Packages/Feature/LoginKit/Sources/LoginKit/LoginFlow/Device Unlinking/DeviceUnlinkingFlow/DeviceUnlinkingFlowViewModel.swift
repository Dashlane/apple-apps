...
public class DeviceUnlinkingFlowViewModel: ObservableObject, LoginKitServicesInjecting {
    enum Alert {
        case unlinkFailed(Set<DeviceListEntry>)
    }
    ...
    func initialStep() {
        ...
        let logger = DeviceLimitUsageLogger(logger: loginUsageLogService,
                                            activityReporter: userTrackingSessionActivityReporter,
                                            mode: mode)
        showLimitedNumberOfDevicesView(mode: mode, logger: logger)
        logger.log(.limitPrompt, action: .seen)
    ...
    func unlinkAndLoadSession(mode: DeviceUnlinkMode, using deviceUnlinker: DeviceUnlinker) {
        ...
        deviceUnlinker.unlink(devices) { [weak self] result in
            ...
            case .failure(let error):
                ...
                self.alert = .unlinkFailed(devices)
            }
        }
    }
