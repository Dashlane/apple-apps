    ...
    var mainView: some View {
        StepBasedContentNavigationView(steps: $model.steps) { step in
            ...
            case let .verification(method, deviceInfo):
                AccountVerificationFlow(model: model.makeAccountVerificationFlowViewModel(method: method, deviceInfo: deviceInfo))
            case let .recoveryKeyInput(authTicket):
                AccountRecoveryKeyLoginView(model: model.makeAccountRecoveryKeyLoginViewModel(authTicket: authTicket))
            case let .changeMasterPassword(masterKey, authTicket):
                NewMasterPasswordView(model: model.makeNewMasterPasswordViewModel(masterKey: masterKey, authTicket: authTicket), title: L10n.Core.accountRecoveryNavigationTitle)
            }
        }.navigationBarStyle(.alternate)
    }

    var errorView: some View {
        FeedbackView(title: L10n.Core.kwExtSomethingWentWrong, message: L10n.Core.recoveryKeyActivationFailureMessage, primaryButton: (L10n.Core.cancel, {
            dismiss()
            model.cancel()
        }))
    }
