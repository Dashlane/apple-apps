...
struct TOTPVerificationView: View {
    ...
    var body: some View {
        ZStack {
            if model.showDuoPush {
                ...
                AuthenticatorPushVerificationView(model: model.makeAuthenticatorPushViewModel(), fallbackOptionTitle: L10n.Core.authenticatorTotpPushOption)
            } else {
                totpView
                    .loginAppearance()
    ...
    private var calloutAndCodeField: some View {
        VStack {
            ...
            DS.TextField(L10n.Core.kwOtpPlaceholderText, text: $model.otp, actions: {
                TextFieldClearContentButton(text: $model.otp)
            })
