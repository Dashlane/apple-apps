...
public struct UntrustedDeviceTransferInfo {
    ...
    public init?(qrCode: String) {
        let cleaned = qrCode.removingPercentEncoding?.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? qrCode
    ...
    public init?(urlComponents: URLComponents) {
        guard urlComponents.path.contains("mplesslogin"),
              ...
              let key = urlComponents.queryItems?[CodingKey.key.rawValue] else {
            return nil
        }
