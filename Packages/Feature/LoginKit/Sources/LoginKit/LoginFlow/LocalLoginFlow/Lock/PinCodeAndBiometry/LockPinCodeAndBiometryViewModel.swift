...
public class LockPinCodeAndBiometryViewModel: ObservableObject, LoginKitServicesInjecting {
    ...
    public var biometricAuthenticationInProgress: Bool {
        switch state {
        case .biometricAuthenticationSuccess, .biometricAuthenticationRequested, .biometricAuthenticationFailure:
            return true
        default:
            return false
        }
...
extension LockPinCodeAndBiometryViewModel {
    static var mock: LockPinCodeAndBiometryViewModel {
        LockPinCodeAndBiometryViewModel(login: Login("John"),
                                        accountType: .invisibleMasterPassword,
                                        pinCodeLock: SecureLockMode.PinCodeLock(code: "123456", attempts: PinCodeAttempts(internalStore: .mock()), masterKey: .masterPassword("Dashlane12")),
...
private extension ActivityReporterProtocol {
    func log(_ state: LockPinCodeAndBiometryViewModel.State?, context: LoginUnlockContext) {
        ...
        switch state {
        case .biometricAuthenticationRequested:
            report(UserEvent.AskAuthentication(mode: .biometric,
        ...
        case .pinRequested:
            report(UserEvent.AskAuthentication(mode: .pin,
                                               reason: context.reason,
                                               verificationMode: context.verificationMode))
        case .pinIncorrect:
            report(UserEvent.Login(isBackupCode: context.isBackupCode,
