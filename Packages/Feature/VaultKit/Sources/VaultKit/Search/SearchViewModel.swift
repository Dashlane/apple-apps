...
open class SearchViewModel {
    ...
    public init(
        ...
        let searchItemsPublisher = itemsPublisher.combineLatest(searchPublisher) { (items, criteria) -> DataSection? in
            guard !criteria.isEmpty else { return nil }
            let filteredItems = items.filterAndSortItemsUsingCriteria(criteria)

            guard !filteredItems.isEmpty else { return nil }
            let sectionTitle =  filteredItems.count > 1 ? L10n.Core.KWVault.Search.Items.Title.plural : L10n.Core.KWVault.Search.Items.Title.singular

            return DataSection(name: sectionTitle, items: filteredItems)
        }

        let searchCollectionsPublisher = collectionsPublisher
            .combineLatest(itemsPublisher, searchPublisher) { [weak self] (collections, items, criteria) -> [DataSection] in
        ...
        searchCollectionsPublisher
            .combineLatest(searchItemsPublisher, searchPublisher) { collectionsSections, itemsSections, criteria in
