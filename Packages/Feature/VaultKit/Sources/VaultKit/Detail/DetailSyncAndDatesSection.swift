...
struct DetailSyncAndDatesSection: View {
    ...
    private var updateDateLabel: String {
        return item.isShared ? L10n.Core.vaultItemModificationDateByYouLabel : L10n.Core.vaultItemModificationDateLabel
    }
...
struct SyncAndDatesSection_Previews: PreviewProvider {
    ...
    static var pendingSyncPreviews: some View {
        ...
        List {
            DetailSyncAndDatesSection(item: .mockPendingSync(creationDate: .now.substract(days: 2), modificationDate: .now))
    ...
    static var syncedPreviews: some View {
        ...
        List {
            DetailSyncAndDatesSection(item: .mockSynced(creationDate: .now.substract(days: 2), modificationDate: .now.substract(days: 1)))
        }
        .previewDisplayName("Updated and synced")

        List {
            DetailSyncAndDatesSection(item: .mockSynced(creationDate: .now.substract(days: 2), modificationDate: .now))
        }
