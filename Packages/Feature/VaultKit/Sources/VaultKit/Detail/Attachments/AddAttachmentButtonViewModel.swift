      ...
      $imageContent
        .sink { imageContent in
          ...
          if self.isRenamingFileEnabled {
            ...
            self.save(
              .fileData(.image(imageContent), filename: L10n.Core.kwDefaultFilename + ".jpeg"))
          }
        }
    ...
    func saveScanImagesIntoPDF(result: Result<[UIImage], Error>) {
      ...
      case let .success(images):
        ...
        if self.isRenamingFileEnabled {
          ...
          save(.fileData(.scan(data: data), filename: L10n.Core.scannedDocumentName + ".pdf"))
        }
      }
    }
  ...
  extension AddAttachmentButtonViewModel.FileData {
    fileprivate var defaultName: String {
      switch self {
      case .scan:
        return L10n.Core.scannedDocumentName + ".pdf"
      case .image:
        return L10n.Core.kwDefaultFilename + ".jpeg"
      }
    }
  }
