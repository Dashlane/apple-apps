...
public class AddAttachmentButtonViewModel: ObservableObject, VaultKitServicesInjecting {
  ...
  public init(
    ...
    if !capabilityService.status(of: .secureFiles).isAvailable {
    ...
    $imageContent
      .sink { imageContent in
        ...
        if self.isRenamingFileEnabled {
          ...
          self.save(.fileData(.image(imageContent), filename: CoreL10n.kwDefaultFilename + ".jpeg"))
        }
      }
  ...
  func saveScanImagesIntoPDF(result: Result<[UIImage], Error>) {
    ...
    case let .success(images):
      ...
      if self.isRenamingFileEnabled {
        ...
        save(.fileData(.scan(data: data), filename: CoreL10n.scannedDocumentName + ".pdf"))
      }
    }
  }
  ...
  private func performSave(_ file: File) throws {
    ...
    case let .document(fileUrl):
      let movedUrl = try self.documentStorageService.documentCache.move(
        fileUrl,
    ...
    case let .fileData(fileData, filename):
      ...
      case let .image(imageData):
        if let url = imageData.fileUrl {
          let movedUrl = try self.documentStorageService.documentCache.copy(
            url,
            to: .decryptedDirectory,
          ...
          let url = try documentStorageService.documentCache.write(
            image,
            to: .decryptedDirectory,
            filename: filename.imageNameWithExtension,
            isUnique: true)
      ...
      case let .scan(data):
        let fileUrl = try documentStorageService.documentCache.write(
          data,
          to: .decryptedDirectory,
  ...
  private func upload(_ fileURL: URL) throws {
    ...
    Task.detached { [weak self] in
      ...
      do {
        try await self.documentStorageService.upload(
          fileURL,
          progress: progress,
...
extension VaultItem {
  var logItemType: Definition.ItemType {
    ...
    case .drivingLicence: return .driverLicense
...
extension AddAttachmentButtonViewModel.FileData {
  fileprivate var defaultName: String {
    switch self {
    case .scan:
      return CoreL10n.scannedDocumentName + ".pdf"
    case .image:
      return CoreL10n.kwDefaultFilename + ".jpeg"
    }
  }
}

extension String {
  fileprivate var imageNameWithExtension: String {
    let filename = NSString(string: self)
    let pathPrefix = filename.deletingPathExtension
    return pathPrefix + ".jpeg"
  }

  fileprivate var pdfDocumentWithExtension: String {
    let filename = NSString(string: self)
    let pathPrefix = filename.deletingPathExtension
