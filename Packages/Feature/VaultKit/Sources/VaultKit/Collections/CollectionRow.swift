...
struct CollectionRow: View {
    ...
    private var itemsCount: some View {
        let count = viewModel.collection.items.count
        Text(count > 1 ? L10n.Core.KWVaultItem.Collections.ItemsCount.plural(count) : L10n.Core.KWVaultItem.Collections.ItemsCount.singular(count))
            .font(.footnote)
            .foregroundColor(.ds.text.neutral.quiet)
            .lineLimit(1)
    }
...
struct CollectionRow_Previews: PreviewProvider {
    static var previews: some View {
        CollectionRow(viewModel: .mock(collection: PersonalDataMock.Collections.business))
    }
}
