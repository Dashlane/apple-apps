...
public struct DomainIconView: View {
    ...
    private var placeholder: some View {
        ...
        PlaceholderIconView(title: placeholderTitle, sizeType: model.size)
            .overlay(style.shape.inset(by: 0.5).stroke(Color.ds.border.neutral.quiet.idle, lineWidth: 1.61))
    }
    
    @ViewBuilder
    private func icon(for image: SwiftUI.Image, colors: IconColorSet?) -> some View {
        let colorSet = colors ?? .placeholderColorSet
        let style = IconStyle(backgroundColor: Color(colorSet.backgroundColor), sizeType: model.size)
        image
            .resizable()
    ...
    private func borderColor(for colorSet: IconColorSet) -> SwiftUI.Color {
        if colorSet.backgroundColor.isBorderRequired() {
            ...
            return Color(colorSet.backgroundColor)
        }
    }
}
...
public extension DomainIconViewModel {
    init(credential: Credential, size: IconStyle.SizeType, iconService: IconServiceProtocol) {
        self.init(domain: credential.url?.domain, size: size, iconService: iconService)
    }
    
    init(personalDataURL: PersonalDataURL, size: IconStyle.SizeType, iconService: IconServiceProtocol) {
