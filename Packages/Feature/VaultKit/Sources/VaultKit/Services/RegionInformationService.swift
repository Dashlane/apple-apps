...
extension Array {
  func firstItem<T: Decodable>(where predicate: (T) throws -> Bool) rethrows -> T?
  where Element == RegionInformationContainer<T> {
    for region in self {
...
extension CallingCodesInformationManager {
  public func code(for country: CountryCodeNamePair) -> CallingCode? {
    return callingCodes.first(where: { $0.region == country.code })
  }

  public func code(forCountry country: String) -> CallingCode? {
