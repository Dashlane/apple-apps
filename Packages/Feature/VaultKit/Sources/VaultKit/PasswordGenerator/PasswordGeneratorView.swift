...
public struct PasswordGeneratorView: View {
    ...
    var trailingView: some View {
        ...
        PasswordGeneratorForm {
            ...
            Section(header: Text(L10n.Core.kwPadExtensionGeneratorLength.uppercased()) + Text(": \(viewModel.preferences.length)")) {
    ...
    var passwordView: some View {
        ...
        Group {
            switch viewModel.mode {
                case let .standalone(action):
                    RoundedButton(L10n.Core.passwordGeneratorCopyButton, action: viewModel.performMainAction)
                ...
                case .selection:
                    RoundedButton(L10n.Core.passwordGeneratorUseButton) {
    ...
    private func makeSavePasswordActionSheet(action: @escaping () -> Void) -> ActionSheet {
        ActionSheet(title: Text(L10n.Core.dwmOnboardingCardPWGTabEmailCopied).foregroundColor(.green),
                    message: Text(L10n.Core.savePasswordMessageNewpassword),
                    buttons: [.default(Text(L10n.Core.kwSave), action: action),
