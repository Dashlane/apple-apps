...
public class UpdateOperatingSystemAnnouncement: HomeModalAnnouncement, HomeAnnouncementsServicesInjecting {
    ...
    public init(informationProvider: DeviceInformationProvider = DeviceInformation(),
...
public class UpdateOperatingSystemCache: UpdateOperatingSystemCacheProtocol {
    ...
    public func canDisplayAnnouncement() -> Bool {
        guard let lastDismissalDate else {
                ...
                guard let oneMonthAfterLastDismissal = Calendar.current.date(byAdding: .month, value: 1, to: lastDismissalDate) else {
            return false
        }
...
extension UpdateOperatingSystemAnnouncement {
    ...
    static func mock(modelName: String = UnsupportedDevice.iPhone7.rawValue,
                     systemWillBeDropped: Bool = true,
                     canDisplayAnnouncement: Bool = false) -> UpdateOperatingSystemAnnouncement {
        .init(informationProvider: DeviceInformationProviderMock(modelName: modelName, systemWillBeDropped: systemWillBeDropped),
        cache: UpdateOperatingSystemCacheMock(canDisplay: canDisplayAnnouncement))
    }

    static func mock(modelName: String = UnsupportedDevice.iPhone7.rawValue,
                     systemWillBeDropped: Bool = true,
                     cache: UpdateOperatingSystemCacheProtocol) -> UpdateOperatingSystemAnnouncement {
        .init(informationProvider: DeviceInformationProviderMock(modelName: modelName, systemWillBeDropped: systemWillBeDropped),
        cache: cache)
    }
}
