        ...
        VStack(spacing: 20) {
            TabView(selection: $selection) {
                ...
                if model.shouldShowSync {
                    pageView(for: .image(Asset.autofillTutorialSync),
            ...
            RoundedButton(L10n.Core.autofillDemoFieldsAction, action: { model.action() })
...
struct AutofillOnboardingIntroView_Previews: PreviewProvider {
    static var previews: some View {
        MultiContextPreview {
            AutofillOnboardingIntroView(model: .init(shouldShowSync: true, activityReporter: .fake, action: {}, dismiss: {}))
        }
    }
}
