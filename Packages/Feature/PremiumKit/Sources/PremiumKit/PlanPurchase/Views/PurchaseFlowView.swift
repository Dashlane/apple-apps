...
public struct PurchaseFlowView: View {
  ...
  private func alert(for dismissAction: PurchaseFlowDismissAction) -> Alert {
    Alert(
      ...
      dismissButton: .default(
        Text(CoreL10n.planScreensOK), action: { self.completion(dismissAction) }))
  }
  ...
  private func makePurchaseView(
    ...
    case .paywall(let trigger):
      switch trigger {
      case .frozenAccount, .capability(key: .passwordsLimit):
        FrozenPaywall(wasOnTrial: model.wasOnTrial) { action in
          model.handlePaywallViewAction(action)
        }
      case let .capability(capability):
        let planGroups = planTiers.firstPlan(for: capability.orderedByPriorityPurchaseKinds)
        if let viewModel = model.makePaywallViewModel(
          capability: capability, purchasePlanGroup: planGroups)
        {
          PaywallView(
...
extension PurchaseFlowDismissAction {
  fileprivate var alertTitle: String {
    ...
    case let .success(plan):
      return CoreL10n.planScreensPurchaseCompleteTitle(plan.localizedTitle)
    default:
      return CoreL10n.planScreensPurchaseErrorTitle
    }
  }
  ...
  fileprivate var alertMessage: String {
    switch self {
    case .success:
      return CoreL10n.planScreensPurchaseCompleteMessage
    case let .failure(error):
      return DiagnosticMode.isEnabled
        ? error.debugDescription : CoreL10n.planScreensPurchaseErrorMessage
    default:
      return CoreL10n.planScreensPurchaseErrorMessage
    }
  }
}
