...
public class AuthenticatorDatabaseService: AuthenticatorDatabaseServiceProtocol, ObservableObject {
    ...
    public func load() {
        persistor.load { (items: PersistedItems<PersistedOTPInformation>?) in
            ...
            codes = Set(items.items.compactMap { item in
                guard let conf = try? OTPConfiguration(otpURL: item.otpURL,
                                                       supportDashlane2FA: true,
                                                       defaultTitle: item.title,
                                                       defaultLogin: item.login,
                                                       defaultIssuer: item.issuer) else {
                    return nil
                }
                return OTPInfo(id: item.id, configuration: conf, isFavorite: item.isFavorite ?? false, recoveryCodes: item.recoveryCodes)
            })
        }
    }
}

public extension PersistedOTPInformation {
