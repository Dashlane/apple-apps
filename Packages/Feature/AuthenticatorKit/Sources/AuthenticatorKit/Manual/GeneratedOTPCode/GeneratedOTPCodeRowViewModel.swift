...
public class GeneratedOTPCodeRowViewModel: ObservableObject {
  ...
  public init(token: OTPInfo, databaseService: AuthenticatorDatabaseServiceProtocol) {
    self.token = token
...
extension OTPInfo {
  ...
  func increasingCounter(by valueToAdd: UInt64 = 1) -> OTPInfo {
    ...
    guard let url = components?.url,
      let newConfiguration = try? OTPConfiguration(
        otpURL: url, defaultTitle: configuration.title, defaultLogin: configuration.login,
        defaultIssuer: configuration.issuer)
    else {
