...
public struct ImportErrorView: View {
    ...
    private func ctaButtons() -> some View {
        switch model.step {
        case .extract:
            RoundedButton(L10n.Core.m2WImportGenericImportErrorScreenBrowse) {
        ...
        case .save:
            VStack {
                RoundedButton(L10n.Core.m2WImportGenericImportErrorScreenTryAgain) {
                    self.save()
                }
                .roundedButtonLayout(.fill)
                .padding(.horizontal, 16)
...
private extension ImportErrorView {
...
private extension ImportErrorView {
    ...
    var attributedHelpInfo: AttributedString {
