...
private extension LastpassItem {
    var hasSecureNotePrefix: Bool {
...
private extension SecureNote {
    init(from lastpassItem: LastpassItem) {
        self.init(title: lastpassItem.name,
                  content: lastpassItem.extra)
    }
}

private extension Credential {
    init(from lastpassItem: LastpassItem, using urlDecoder: PersonalDataURLDecoderProtocol) {
        self.init(
