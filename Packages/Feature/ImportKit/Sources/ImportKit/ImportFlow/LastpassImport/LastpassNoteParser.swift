  ...
  private static func parse(
    ...
    case NoteTypeValue.bankAccount.rawValue:
      ...
      lines.forEach { line in
        if let header = BankAccountHeader.allCases.map(\.rawValue).first(where: {
          line.hasPrefix($0)
        }) {
          let value =
            line
    ...
    case NoteTypeValue.creditCard.rawValue:
      var result: [String: String] = [:]
      lines.forEach { line in
        if let header = CreditCardHeader.allCases.map(\.rawValue).first(where: {
          line.hasPrefix($0)
        }) {
          let value =
            line
...
extension CreditCard {
  fileprivate static func create(from dic: [String: String]) -> CreditCard? {
...
extension BankAccount {
  fileprivate static func create(from dic: [String: String]) -> BankAccount? {
