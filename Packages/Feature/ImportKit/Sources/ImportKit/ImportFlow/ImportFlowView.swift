...
public struct ImportFlowView<Model: ImportFlowViewModel>: View {
  ...
  public var body: some View {
    ...
    StepBasedContentNavigationView(steps: $viewModel.steps) { step in
      ...
      case .list(let model):
        if let model = model as? Model.AnyImportViewModel {
          ...
          fatalError(
            "\(model) was created with a different kind of ImportViewModel, or does not conform to ObservableObject"
          )
        }
    ...
    .onCSVFileDrop(
      isDroppingFileEnabled: viewModel.isDroppingFileEnabled,
      dropper: dropper)
  }
}
...
extension View {
  @ViewBuilder
  fileprivate func onCSVFileDrop(isDroppingFileEnabled: Bool, dropper: Dropper) -> some View {
...
private struct Dropper: DropDelegate {
  ...
  func performDrop(info: DropInfo) -> Bool {
    ...
    item.loadItem(forTypeIdentifier: UTType.commaSeparatedText.identifier, options: nil) {
      (urlData, _) in
