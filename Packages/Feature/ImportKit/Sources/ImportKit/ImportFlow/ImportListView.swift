...
public struct ImportListView<Model>: View where Model: ObservableObject, Model: ImportViewModel {
    ...
    public var body: some View {
        ...
        .confirmationDialog(L10n.Core.teamSpacesSharingAcceptPrompt,
    ...
    private var passwordList: some View {
        ScrollView {
            ForEach(model.items) { item in
                ImportItemRowView(item: item, iconViewModel: .init(item: item.vaultItem, iconLibrary: model.iconService.domain))
            }
            .padding(.leading, 16)
            .padding(.trailing, 24)
    ...
    var importButtonBackground: some View {
        ...
        LinearGradient(gradient: Gradient(colors: colors), startPoint: .bottom, endPoint: .top).edgesIgnoringSafeArea(.bottom)
    }
    ...
    private func save() {
        Task {
            ...
            } catch ImportViewModelError.needsB2BSpace {
