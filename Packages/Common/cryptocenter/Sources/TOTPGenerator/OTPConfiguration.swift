...
public struct OTPConfiguration: Equatable, Hashable, Codable {
    enum Key: String {
        case secret
        case issuer
        case algorithm
    ...
    public init(otpString: String,
                supportDashlane2FA: Bool = false) throws {
                                let cleaned = otpString.removingPercentEncoding?.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? otpString
    ...
    public init(otpURL: URL,
        ...
        var secretData: Data! = nil

        let nonDigitsCharacterSet = CharacterSet.decimalDigits.inverted
        if let _ = secret.rangeOfCharacter(from: nonDigitsCharacterSet) {
            guard let data = secret.base32DecodedData as Data? else {
        ...
        } else if let issuerFromLabel = components.path.components(separatedBy: "/").last?.components(separatedBy: ":").first {
    ...
    public var issuerURL: URL? {
        if let url = URL(string: title) { return url }
        else if let issuer = issuer, let url = URL(string: issuer) { return url }
        return nil
    }
}

public extension OTPConfiguration {
    static var mock: OTPConfiguration {
        return try! OTPConfiguration(otpURL: URL(string:"otpauth://totp/Test%20Issuer:test%40test.com?secret=yzg6fpgoazbjlhyb3fcilky3dj2oz3ad27dlbtrbf2vqfajazo7albwj&algorithm=SHA256&digits=6&period=30&lock=false")!)
    }
}
