...
public struct DocumentCache {
        ...
        return try FileManager.default.cacheDirectory()
            .appendingPathComponent("documentStorageCache",
                                    isDirectory: true)
    }

        private func encryptedFilesURL() throws -> URL {
        return try cacheURL().appendingPathComponent("encryptedFiles",
                                                     isDirectory: true)
    }

        private func decryptedFilesURL() throws -> URL {
        return try cacheURL().appendingPathComponent("decryptedFiles",
                                                   isDirectory: true)
    }

        init() {
        try? clear(.allDirectories)
    }
        ...
        return cacheURL
                                        ...
                                        private func destination(scope: CacheScope, filename: String, isUnique: Bool = false) throws -> URL {
        var destination: URL
        switch scope {
        case .allDirectories:
        ...
        return destination
                    ...
                    public func urlOfEncryptedDirectory(with fileComponent: String? = nil,
