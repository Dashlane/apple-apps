...
public extension DocumentAttachable {
                mutating func updateAttachments(with secureFileInfo: SecureFileInformation) throws {
        let newAttachment = try Attachment(from: secureFileInfo)
        self.updateAttachments(with: newAttachment)
    }

                mutating func updateAttachments(with newAttachment: Attachment) {
        var keepingAttachments: Set<Attachment> = attachments?.filter { $0.id != newAttachment.id } ?? []
...
extension Attachment {
        ...
        guard let version = Int(secureFileInfo.version),
              ...
              let userModificationTimeStamp: Double = secureFileInfo.userModificationDatetime?.timeIntervalSince1970
        else {
            throw AttachmentUpdateError("This secure file information is missing some information")
        }
        self.init(id: secureFileInfo.id.rawValue,
                  version: version,
