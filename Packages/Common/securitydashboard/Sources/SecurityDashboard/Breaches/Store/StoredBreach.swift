...
public struct StoredBreach {
  ...
  func updated(with leakedPasswords: Set<BreachesService.Password>) -> StoredBreach {
    return StoredBreach(
      storedBreach: self, leakedPasswords: self.leakedPasswords.union(leakedPasswords))
  }

  func leakedPasswords(for authentifiants: [SecurityDashboardCredential]) -> Set<
    BreachesService.Password
  > {
    ...
    let breachedPasswords = Set(
      authentifiants
        .filter {
          guard let domain = $0.domain else { return false }
          guard breach.domains().contains(domain) else { return false }
...
extension StoredBreach {
  func updated(with info: DataLeakInfo) -> StoredBreach {
    let newBreach = info.data.reduce(into: StoredBreach(storedBreach: self, information: [info])) {
      (result, info) in
      switch info.type {
      case .password:
