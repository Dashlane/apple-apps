...
extension AlertBuildersUtils {
    ...
    static func generateTitle(for alertData: AlertGenerator.AlertData, and localizationProvider: LocalizationProvider, details: AlertTitle) throws -> String {
...
private extension Breach {
    func title(using key: LocalizationKey, and localizationProvider: LocalizationProvider) throws -> String {
...
extension AlertBuildersUtils {
    ...
    struct UserDataDetails {
        ...
        let usernamesKey: LocalizationKey
                                            ...
                                            static func generateUserDataDetails(for alertData: AlertGenerator.AlertData,
                                        and localizationProvider: LocalizationProvider,
                                        using details: UserDataDetails,
                                        and dataInvolvedDetails: DataInvolvedDetails? = nil) throws -> AlertSection? {

        let maximumNumberOfElements = 5

        func generateSection(for data: Set<String>, using key: LocalizationKey) -> AlertSection? {
            guard data.count > 0 else { return nil }
            return AlertSection(title: AlertSection.Title(localizationProvider.localizedString(for: key)), contents: [data.joined(separator: ", ")])
        }

                let emails = Set((alertData.impactedCredentials.compactMap({ $0.email }) + (alertData.breach.impactedEmails ?? [])).filter({ !$0.isEmpty }).prefix(maximumNumberOfElements))
                let domains = Set((alertData.impactedCredentials.compactMap({ $0.domain }) + (alertData.breach.domains ?? [])).filter({ !$0.isEmpty }).prefix(maximumNumberOfElements))

                let dataInvolved: AlertSection? = try {
            guard let dataInvolvedDetails = dataInvolvedDetails else { return nil }
            guard let dataInvolvedSection = try generateDataInvolvedDetails(for: alertData, and: localizationProvider, details: dataInvolvedDetails) else {
                return nil
            }
            let alertSection = AlertSection(title: dataInvolvedSection.title, contents: [dataInvolvedSection.contents.joined(separator: ", ")])
            return alertSection
        }()

                let usernames: Set<String> = {
            guard alertData.breach.leakedData?.contains(.username) ?? false else { return [] }
            return Set(alertData.impactedCredentials.compactMap({ $0.username }).filter({ !$0.isEmpty }).prefix(maximumNumberOfElements))
        }()
        ...
        let finalSection = sections
            .compactMap({ $0 })
            .reduce(into: AlertSection(title: AlertSection.Title(""), contents: [])) { (result, section) in
...
extension AlertBuildersUtils {
    ...
    struct DataInvolvedDetails {
        let titleKey: LocalizationKey
                        ...
                        static func generateDataInvolvedDetails(for alertData: AlertGenerator.AlertData,
                                            and localizationProvider: LocalizationProvider,
                                            details: DataInvolvedDetails) throws -> AlertSection? {
        guard let leakedData = alertData.breach.leakedData, !leakedData.isEmpty else {
            throw AlertGenerator.AlertError.breachDoesNotHaveLeakedData
        }

        let leakedDataStrings = leakedData
            .filter { $0 != .unknown }
            .compactMap { LocalizationKey.key(for: $0) }
            .compactMap { localizationProvider.localizedString(for: $0) }
        return AlertSection(title: AlertSection.Title(localizationProvider.localizedString(for: details.titleKey)),
...
extension AlertBuildersUtils {
    ...
    struct CompromisedAccountsRecommendation {
        ...
        let multipleCompromisedKey: LocalizationKey
                                                                ...
                                                                static func generateCompromisedAccountsRecommendation(for alertData: AlertGenerator.AlertData,
                                                          and localizationProvider: LocalizationProvider,
                                                          details: CompromisedAccountsRecommendation) throws -> AlertSection? {
        ...
        case 0:
            ...
            case .dataLeakAlertWithCompromisedPiis:
                return AlertSection(title: AlertSection.Title(localizationProvider.localizedString(for: details.onlyPIIsCompromisedKey)),
                                    contents: [])
            case .dataLeakAlertWithCompromisedPasswords, .dataLeakAlertWithCompromisedPasswordsAndPiis:
                return AlertSection(title: AlertSection.Title(localizationProvider.localizedString(for: details.noPasswordCompromisedKey)),
                                    contents: alertData.breach.domains ?? [])
            default:
                return nil
            }
        case 1:
            return AlertSection(title: AlertSection.Title(localizationProvider.localizedString(for: details.oneCompromisedKey)),
                                contents: ["\(alertData.numberOfCompromisedCredentials)"])
        default: 
            return AlertSection(title: AlertSection.Title(localizationProvider.localizedString(for: details.multipleCompromisedKey)),
                                contents: ["\(alertData.numberOfCompromisedCredentials)"])
        }
    }
}
