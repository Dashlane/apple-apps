...
struct DataLeakPlaintextTrayAlertBuilder: TrayAlertBuilderProtocol {
  ...
  func build() throws -> DataLeakPlaintextTrayAlert {
    ...
    let leakedPasswords = try self.generateLeakedPasswordsString(
      for: data, and: localizationProvider)
    let recommendation = try self.generateRecommendation(for: data, and: localizationProvider)

    return DataLeakPlaintextTrayAlert(
      title: title,
      description: nil,
      details: details,
...
extension DataLeakPlaintextTrayAlertBuilder {
  ...
  func generateDetails(
    ...
    let affectedEmails = String(
      format: localizationProvider.localizedString(for: .popupBreachDetailsEmails),
...
extension DataLeakPlaintextTrayAlertBuilder {
  ...
  func generateLeakedPasswordsString(
    for alertData: AlertGenerator.AlertData, and localizationProvider: LocalizationProvider
  ) throws -> AlertSection? {
    guard alertData.alertType == .dataLeakAlertWithLeakedData,
      alertData.numberOfCompromisedCredentials > 0
    else { return nil }
    ...
    let string = String(
      format: localizationProvider.localizedString(for: .trayAffectedPasswords), joinedPasswords)
...
extension DataLeakPlaintextTrayAlertBuilder {
  ...
  func generateRecommendation(
    ...
    let recommendation = String(
      format: localizationProvider.localizedString(
        for: .trayAffectedPasswordsMatchingCredentialsRecommandation), parameter)
    return AlertSection(title: .init(recommendation), contents: ["\(parameter)"])
  }

  func noCredentialImpactedRecommendation(
    for alertData: AlertGenerator.AlertData, and localizationProvider: LocalizationProvider
  ) -> AlertSection? {
    guard let parameter = alertData.breach.impactedEmails.first,
      alertData.impactedCredentials.isEmpty
    else {
      return nil
    }
    ...
    let recommendation = String(
      format: localizationProvider.localizedString(
        for: .trayRecommendationDataLeakNoDomainNoPassword), parameter)
    return AlertSection(title: .init(recommendation), contents: ["\(parameter)"])
  }
}
