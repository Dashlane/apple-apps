...
extension DashlaneSixPopupAlertBuilder {
  ...
  func generateDescription(
    ...
    let domains = breach.domains().joined(separator: ", ")
    ...
    if alertData.alertType.isDataLeakAlert {
      let impactedEmails = breach.impactedEmails.joined(separator: ", ")
      guard !impactedEmails.isEmpty else {
        throw AlertGenerator.AlertError.breachDoesNotHaveImpactedEmail
      }
      return AlertSection(
        title: AlertSection.Title(
          localizationProvider.localizedString(for: .popupDataLeakDescription)),
        contents: [domains, breachDate, impactedEmails])
    } else {
      return AlertSection(
        title: AlertSection.Title(
          localizationProvider.localizedString(for: .popupRegularDescription)),
...
extension DashlaneSixPopupAlertBuilder {
  ...
  func generateRecommendation(
    ...
    let details = AlertBuildersUtils.CompromisedAccountsRecommendation(
      onlyPIIsCompromisedKey: .popupRecommendationOnlyPII,
    ...
    return try AlertBuildersUtils.generateCompromisedAccountsRecommendation(
