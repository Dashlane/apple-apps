...
public protocol DataLeakInformationDataDecryptor {
  func decrypt(_ data: Data, using encryptedCipherKey: Data) throws -> Data
}
...
public actor BreachesService: Cancelable {
  ...
  private let cachedBreachCredentialsPublisher:
    AnyPublisher<CredentialsService.CredentialsByBreachId, Never>
  ...
  init(
    ...
    self.fetcher = BreachesFetcher(userDeviceAPIClient: userDeviceAPIClient, logger: logger)
    Task {
  ...
  private func fetchRemotePublicBreachesAndUpdateStore(
    for credentials: [SecurityDashboardCredential]
  ) async {
    let latestRevision = self.breachesStore.lastRevisionForPublicBreaches
    let result = await self.fetcher.fetchBreaches(
      existingBreaches: self.cachedBreaches.value, userCredentials: credentials,
      latestRevision: latestRevision)
    self.breachesStore.lastRevisionForPublicBreaches = result.revision
    self.updateStoreAndCache(with: result.delta)
  }

  private func fetchRemoteDataLeaksAndUpdateStore(
    for credentials: [SecurityDashboardCredential],
    using decryptor: DataLeakInformationDataDecryptor?
  ) async throws {
    let lastUpdateDate = self.breachesStore.lastUpdateDateForDataLeakBreaches
    do {
      let leaksResult = try await self.fetcher.fetchDataLeaks(
        existingBreaches: self.cachedBreaches.value,
        decryptor: decryptor,
  ...
  static func treat(
    _ newBreaches: Set<StoredBreach>,
    comparingAgainst existingBreaches: Set<StoredBreach>,
    using credentials: [SecurityDashboardCredential]
  ) -> DeltaUpdateBreaches {
    return BreachesFilter.delta(
      forOnlineBreaches: newBreaches,
      existingBreaches: existingBreaches,
  ...
  private func updateStoreAndCache(with delta: DeltaUpdateBreaches) {
    ...
    if !delta.inserted.isEmpty {
      self.log.debug(
        "Will try to store \(delta.inserted.count) breaches - \(delta.inserted.map({ $0.breachID }))"
      )
    ...
    if !delta.updated.isEmpty {
      self.log.debug(
        "Will try to update \(delta.updated.count) breaches - \(delta.updated.map({ $0.breachID }))"
      )
  ...
  private func pendingBreaches() -> Set<StoredBreach> {
    return Set(
      self.cachedBreaches.value.filter({ $0.status == .pending }).filter({
        $0.breach.status != .deleted
      }))
  }

  private func pendingAndViewedBreaches() -> Set<StoredBreach> {
    return Set(
      self.cachedBreaches.value.filter({ $0.status == .pending || $0.status == .viewed }).filter({
        $0.breach.status != .deleted
      }))
  }

  public func trayAlerts(for credentials: [String: [SecurityDashboardCredential]])
    -> [TrayAlertProtocol]
  {
    self.pendingAndViewedBreaches().compactMap { storedBreach -> TrayAlertProtocol? in
      ...
      return try? AlertGenerator.tray(
        for: storedBreach.breach,
  ...
  public nonisolated func trayAlertsPublisher(breachPublisher: AnyPublisher<[StoredBreach], Never>)
    -> AnyPublisher<[TrayAlertProtocol], Never>
  {
    breachPublisher
      ...
      .map { cachedBreaches, cachedBreachesCredentials -> [PopupAlertProtocol] in
        cachedBreaches
          ...
          .compactMap { [weak self] cachedBreach -> PopupAlertProtocol? in
            ...
            return try? AlertGenerator.popup(
              for: cachedBreach.breach,
              compromising: credentials,
...
extension Sequence where Element == StoredBreach {
  fileprivate func breachesByPasswords() -> [BreachesService.Password: Set<Breach>] {
...
extension StoredBreach.Status {
  fileprivate var isDisplayable: Bool {
