...
struct ReusedFilterAlgorithm: FilterAlgorithm {
        ...
        public mutating func filter(bySpacedId spaceId: String?, sensitiveOnly: Bool) {
            elements = ReusedFilterAlgorithm.filter(for: elements, bySpaceId: spaceId, sensitiveOnly: sensitiveOnly)
        }
    }

    	static func compute(_ credentialsToUse: [SecurityDashboardCredential], using services: PasswordHealthAnalyzerServices) async -> PasswordHealthResult {
        ...
        let elements = credentialsToUse.enumerated().filter { indices.contains($0.offset) }.map { $0.element }
