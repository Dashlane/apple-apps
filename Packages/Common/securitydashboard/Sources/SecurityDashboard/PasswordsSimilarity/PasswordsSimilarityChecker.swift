...
struct PasswordsSimilarityChecker {
  ...
  mutating func `is`(
    ...
    minimumPasswordSize minimumSize: Int = Self.defaultMinCharCount
  ) -> Bool {
    ...
    let distance = Levenshtein.distance(
      between: source, and: target, limitedBy: maxLevenshteinDistance)
  ...
  mutating func groupByIndices(
    _ passwords: [Password],
    usingMaxLevenshteinDistance maxLevenshteinDistance: Int = Self.defaulMaxLevenshteinDistance
  ) -> [IndicesGroup] {

    var groups = [Pair].init(
      repeating: (0, Bitset.init(size: passwords.count)), count: passwords.count)
    ...
    for i in 0..<passwords.count {
      ...
      where self.is(
        passwords[i], equivalentTo: passwords[j],
    ...
    for group in groups {
      let finalGroup = passwords.enumerated().compactMap {
        group.bitset[$0.offset] ? $0.offset : nil
      }
      if finalGroup.count > 1 {
        result.append(
          IndicesGroup(
            possibleRoot: group.value, count: finalGroup.count, linkedIndices: finalGroup))
      }
    }
  ...
  mutating func group(
    ...
    let groups: [Group] = groupByIndices(
      passwords, usingMaxLevenshteinDistance: maxLevenshteinDistance
    )
    .map {
      (
        passwords[$0.possibleRoot],
  ...
  mutating func similarityCount(
    ...
    for matchingPassword in dataSet
    where self.is(
      password,
...
extension PasswordsSimilarityChecker.IndicesGroup: Equatable {
  static func == (
    lhs: PasswordsSimilarityChecker.IndicesGroup, rhs: PasswordsSimilarityChecker.IndicesGroup
  ) -> Bool {
